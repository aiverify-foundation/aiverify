import { transformTemplateOutputToState, TemplateOutput } from '../transformTemplateOutputToState';
...
describe('transformTemplateOutputToState', () => {
  ...
  describe('with empty template', () => {
    it('should return default initial state when template has no pages', () => {
      const emptyTemplate: TemplateOutput = {
        ...mockTemplate,
        pages: [],
      };

      const result = transformTemplateOutputToState(emptyTemplate, [mockPlugin]);
    ...
    it('should return default initial state when template pages is null', () => {
      const nullPagesTemplate: TemplateOutput = {
        ...mockTemplate,
        pages: null as any,
      };

      const result = transformTemplateOutputToState(nullPagesTemplate, [mockPlugin]);
  ...
  describe('with valid template', () => {
    it('should transform template with single page correctly', () => {
      const result = transformTemplateOutputToState(mockTemplate, [mockPlugin]);
    ...
    it('should transform template with multiple pages correctly', () => {
      const multiPageTemplate: TemplateOutput = {
        ...mockTemplate,
        pages: [
      ...
      const result = transformTemplateOutputToState(multiPageTemplate, [mockPlugin]);
    ...
    it('should handle widgets with null properties', () => {
      const templateWithNullProperties: TemplateOutput = {
        ...mockTemplate,
        pages: [
          {
            ...mockTemplate.pages[0],
            reportWidgets: [
              {
                ...mockTemplate.pages[0].reportWidgets[0],
                properties: null,
              },
            ],
          },
      ...
      const result = transformTemplateOutputToState(templateWithNullProperties, [mockPlugin]);

      const transformedWidget = result.widgets[0][0];
      expect(transformedWidget.properties).toBeNull();
    });

    it('should use plugin data for widget properties', () => {
      const templateWithMinimalData: TemplateOutput = {
        ...mockTemplate,
        pages: [
          {
            ...mockTemplate.pages[0],
      ...
      const result = transformTemplateOutputToState(templateWithMinimalData, [mockPlugin]);
    ...
    it('should set testResultId and inputBlockDataId to undefined for templates', () => {
      const mockAlgorithm: Algorithm = {
        cid: 'algo1',
        gid: 'algo-plugin',
        name: 'Test Algorithm',
      ...
      expect(result.gridItemToInputBlockDatasMap['widget1'][0]).toEqual({
        gid: 'input-plugin',
        cid: 'input1',
    ...
    it('should avoid duplicate algorithms in algorithmsOnReport', () => {
      ...
      // Create a template with multiple widgets that reference the same algorithm
      const multiWidgetTemplate: TemplateOutput = {
        ...mockTemplate,
        pages: [
          {
            ...mockTemplate.pages[0],
            layouts: [
              { i: 'widget1', x: 0, y: 0, w: 6, h: 4, maxW: 12, maxH: 12, minW: 2, minH: 2, static: false },
              { i: 'widget2', x: 6, y: 0, w: 6, h: 4, maxW: 12, maxH: 12, minW: 2, minH: 2, static: false },
            ],
            reportWidgets: [
              mockTemplate.pages[0].reportWidgets[0],
              {
      ...
      const result = transformTemplateOutputToState(multiWidgetTemplate, [mockPlugin]);
    ...
    it('should avoid duplicate input blocks in inputBlocksOnReport', () => {
      ...
      // Create a template with multiple widgets that reference the same input block
      const multiWidgetTemplate: TemplateOutput = {
        ...mockTemplate,
        pages: [
          {
