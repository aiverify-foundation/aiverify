      ...
      if (xhr.status === 200) {
        try {
          ...
          if (xhr.getResponseHeader('Content-Type')?.includes('application/json')) {
            ...
            reject(new Error('Expected JSON response, but received something else'));
          }
        } catch (err) {
        ...
        if (xhr.status === 413) {
          const errorMessage = 'Body exceeded 1 MB limit. To configure the body size limit for Server Actions, see: https://nextjs.org/docs/app/api-reference/next-config-js/serverActions#bodysizelimit';
