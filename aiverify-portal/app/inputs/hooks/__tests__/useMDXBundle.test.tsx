...
describe('useMDXBundle', () => {
  ...
  it('successfully fetches MDX bundle', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/plugins/test-gid/bundle/test-cid');
  });
  ...
  it('handles API error with status text', async () => {
    ...
    expect((result.current.error as Error).message).toBe('Failed to fetch MDX bundle: Not Found');
  });
  ...
  it('refetches when parameters change', async () => {
    ...
    expect(global.fetch).toHaveBeenLastCalledWith('/api/plugins/new-gid/bundle/new-cid');
  });

  it('calls getMDXBundle function correctly', async () => {
    (global.fetch as jest.MockedFunction<typeof fetch>).mockResolvedValueOnce({
      ok: true,
      json: async () => mockMdxBundle,
    } as Response);
  ...
  it('handles different gid and cid combinations', async () => {
    (global.fetch as jest.MockedFunction<typeof fetch>).mockResolvedValueOnce({
      ok: true,
      json: async () => mockMdxBundle,
    } as Response);

    const { result } = renderHook(() => useMDXBundle('plugin-group', 'widget-component'), {
      wrapper: createWrapper(),
    });
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/plugins/plugin-group/bundle/widget-component');
  });

  it('handles special characters in gid and cid', async () => {
    (global.fetch as jest.MockedFunction<typeof fetch>).mockResolvedValueOnce({
      ok: true,
      json: async () => mockMdxBundle,
    } as Response);

    const { result } = renderHook(() => useMDXBundle('test-group@1.0', 'test-component#2'), {
      wrapper: createWrapper(),
    });
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/plugins/test-group@1.0/bundle/test-component#2');
  });
}); 