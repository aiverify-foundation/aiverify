...
export default async function TemplateDesignerPage({ params }: Props) {
  ...
  console.log("Template API Response:", JSON.stringify(templateResult, null, 2));
  ...
  const mappedPages = template.pages.map((page, pageIndex) => {
    const mappedWidgets = page.reportWidgets.map(templateWidget => {
      ...
      if (!plugin) {
        console.warn(`No plugin found for template widget gid: ${templateWidget.widgetGID}`);
        return null;
      }

      // Find the widget in the plugin's widgets array
      const widget = plugin.widgets.find(w => w.gid === gid && w.cid === cid);
      if (!widget) {
        console.warn(`No widget found in plugin for gid: ${gid} and cid: ${cid}`);
        return null;
      }
      ...
      const algorithmDependencies = widget.dependencies
        ?.map(dep => ({
          gid: dep.gid || gid,
          cid: dep.cid
        })) || [];
