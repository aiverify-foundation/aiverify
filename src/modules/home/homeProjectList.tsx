...
export default function HomeProjectListComponent({ projects }: ProjectListProps) {
	...
	useEffect(() => {
		...
		const filteredProjects = projects.reduce<Project[]>((collection, current) => {
			if (current.report == undefined && filters.indexOf(ProjectReportStatus.NotGenerated) > -1) {
	...
	return (
		<Stack>
			<Box sx={{ display:'flex', alignItems:'center', mb: '25px'}}>
				...
				<Typography variant='heading4' sx={{ mt:0.5 }}>{content.projectsListHeading}</Typography>
			</Box>
			<div style={{ display: 'flex', justifyContent: 'space-between'}}>
				<div>
					<OutlinedInput
						...
						<Button variant="text" 
								sx={{ textTransform: 'none', width: '80px', height: '36px', color: theme.palette.text.strong, fontSize: '14px', ml: '5px' }}
					...
					<div>
						...
						<Chip
							...
							variant={filters.indexOf(ProjectReportStatus.ReportGenerated) > -1 ? 'filled' : 'outlined'}
							color='default'	sx={{ mr: '5px' }} clickable
							onClick={() => handleFilterBtnClick(ProjectReportStatus.ReportGenerated)} />
						<Chip
							label={content.reportNotGenerated}
							variant={filters.indexOf(ProjectReportStatus.NotGenerated) > -1 ? 'filled' : 'outlined'}
							color='default' sx={{ mr: '5px' }} clickable
							onClick={() => handleFilterBtnClick(ProjectReportStatus.NotGenerated)}/>
						<Chip	
							label={content.runningTests}
							variant={filters.indexOf(ProjectReportStatus.RunningTests) > -1 ? 'filled' : 'outlined'}
							color='default' sx={{ mr: '5px' }} clickable
							onClick={() => handleFilterBtnClick(ProjectReportStatus.RunningTests)}/>
						<Chip
							label={content.generatingReport}
							variant={filters.indexOf(ProjectReportStatus.GeneratingReport) > -1 ? 'filled' : 'outlined'}
							color='default' sx={{ mr: '5px' }} clickable
							onClick={() => handleFilterBtnClick(ProjectReportStatus.GeneratingReport)}/>
					</div>
			</div>
			<Grid container spacing={2}>
				{myProjects.map(project => (
					<ProjectBox
						key={`${project.id}-box`} project={project}
				...
				<Grid xs={4} >
					<Paper sx={{ display:'flex', borderRadius: '8px' }}>
						<Box
							sx={{
								p:2, height:'260px', flexGrow:1, display:'flex',
								flexDirection:'column', justifyContent: 'center', alignItems: 'center' }}>
									<div
										...
										<AddCircleOutlineOutlinedIcon sx={{ fontSize: '45px', color: '#d2d2d2'}}/>
										<p style={{ textAlign: 'center', marginTop: '6px' }}>
											<span style={{ fontWeight: 'bold', marginTop: '8px'}}>Create New Project</span><br/>
											<span style={{ fontSize: '13px' }}>Start a new project to test an AI Model and generate reports</span>
										</p>
									</div>
							</Box>
					</Paper>
