...
function SelectedGridItemBoxOutline({ el }: {el : HTMLDivElement}) {
  ...
  const outlineTopStyle: React.CSSProperties = { top, left, width, height: `${OUTLINE_SIZE}px` };
  const outlineRightStyle: React.CSSProperties = { top, left: `calc(${left} + ${width} - ${OUTLINE_SIZE}px)`, width: `${OUTLINE_SIZE}px`, height };
  const outlineBottomStyle: React.CSSProperties = { top: `calc(${top} + ${height} - ${OUTLINE_SIZE}px)`, left, width, height: `${OUTLINE_SIZE}px` };
  const outlineLeftStyle: React.CSSProperties = { top, left, width: `${OUTLINE_SIZE}px`, height };
  ...
  if (el.tagName.toLowerCase() !== 'div' && !el.classList.contains(GRID_ITEM_CLASSNAME)) {
    console.error('Selected grid item outline box not drawn - INVALID-GRID-ITEM');
    return null;
  }
  ...
  return isTransitioning ? null : 
    <>
      ...
      <div className={styles.canvas_item_highlight} style={outlineBottomStyle}/>
