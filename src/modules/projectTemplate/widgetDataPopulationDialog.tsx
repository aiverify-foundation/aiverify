...
import {
  AlertBox, AlertBoxSize,
  DraggableAbsolutionPositon
} from 'src/components/alertBox';
import { ReportWidgetItem, GlobalVariable } from 'src/types/projectTemplate.interface';
...
function InputHelperTooltip({ text }: { text: string }) {
  ...
  return <div style={{ position: 'relative' }}>
    <div onMouseOver={() => setShowHelper(true)} onMouseOut={() => setShowHelper(false)}>
      <InfoIcon style={{ fontSize: '17px', color: '#b8b1b1' }}/>
    </div>
    {showHelper ? <div style={{
      position: 'absolute',
...
export default function WidgetDataPopulationDialog(props: WidgetPopulateDataDialogProps) {
  const {
    reportWidget,
    combinedGlobalVars,
    defaultPosition,
  ...
  const [ selectedProperty, setSelectedProperty ] = useState<UserDefinedProperty>();
  ...
  function handleUseGlobalVarsClick(prop: UserDefinedProperty) {
    return () => {
      if (showGlobalVarsMenu && (selectedProperty && selectedProperty.key === prop.key)) {
        setShowGlobalVarsMenu(false);
        return;
      }
      setSelectedProperty(prop);
  ...
  function handleOnChange(prop: UserDefinedProperty) {
    return (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target)
        handleChangeProperty(prop, e.target.value);
      }
  }

  function handleGlobalVarSelect(prop: UserDefinedProperty | undefined, gVar: GlobalVariable) {
    return () => {
      if (!prop) return;
    ...
    <AlertBox
      ...
      <AlertBox.Header isDragHandle>
        <div className={styles.headingContainer}>
          ...
          <div className={styles.dragIconWrapper}><DragIndicatorIcon style={{ opacity: 0.3 }}/></div>
        </div>
      </AlertBox.Header>
      <AlertBox.Body hasFooter>
        <div style={{ fontSize: '16px'}}>Widget: <span style={{ fontWeight: 'bold' }}>{reportWidget.widget.name}</span></div>
        <p style={{ fontSize: '14px'}}>Fill these input field(s) to populate the text content or values, on the widget. <br/> You can use variables. Add new variables on the Global Variables panel.</p>
        <div>
        ...
        {properties && properties.map(prop => {
          ...
          if (value && value.startsWith('{') && value.endsWith('}')) {
            variable = value;
            value = getPropertyValueFn ? getPropertyValueFn(variable) : value;
          }
          const isPropertyFocused = selectedProperty ? selectedProperty.key : undefined;
          return <div key={`${reportWidget.key}-${prop.key}`} className={styles.propertyInputContainer}>
            <div className={styles.propertyInputField}>
              <div style={{
                display: 'flex',
                justifyContent:'space-between',
                width: 450}}>
                <label style={{ textTransform: 'capitalize', fontSize: 14 }}>{prop.key}</label>
                { Boolean(prop.helper) ? <InputHelperTooltip text={prop.helper as string}/> : null }
              </div>
              <div style={{ display: 'flex' }}>
                <OutlinedInput
                  ...
                  style={{
                    ...
                    flexGrow: 2,
                    outline: 'none',
                    border: prop.key === isPropertyFocused ? '1px solid rgb(75, 37, 90)': 'none'
                  }}/>
                <button
                  className={clsx(
                  ...
                  onClick={handleUseGlobalVarsClick(prop)}>{variable || 'select'}</button>
              </div>
            </div>
          </div>
        })}
      ...
      <AlertBox.Footer>
        <div className={styles.footerContent}>
          <button className="aivBase-button aivBase-button--secondary aivBase-button--small"
            onClick={handleCloseClick}>OK</button>
        </div>
      </AlertBox.Footer>
      {showGlobalVarsMenu ? <div className={styles.variablesMenu}>
        <div style={{ 
          background: '#702F8A',
          color: '#FFFFFF',
          padding: '5px 10px' }}>Global Variables</div>
        <ListMenu>
          {combinedGlobalVars.map(globalVar => <ListMenuItem
