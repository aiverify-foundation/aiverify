...
export default function DisplayInputBlocks ({ plugin }: Props) {
  ...
  return (
    <div className={styles.componentDetails}>
      <div>
        {plugin.inputBlocks ? plugin.inputBlocks.map(ib => (
          <div className={clsx('pluginDetails-card', styles.componentCard)} key={`algo-${ib.gid}`}>
            <h2 className={styles.detailsHeading}>{ib.name} {ib.version}</h2>
            <p className={styles.componentDesc}>{ib.description}</p>
            <div className={styles.valueDisplay}>
              <div className={styles.label}>GID:</div>
              <div className={styles.value} style={{ color: '#991E66'}}>{ib.gid}</div>
            </div>
            <div className={styles.valueDisplay}>
            ...
            {ib.tags ? <div className={styles.valueDisplay}>
