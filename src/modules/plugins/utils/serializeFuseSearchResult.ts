...
import AIFPlugin, { ReportWidget, InputBlock, Algorithm } from 'src/types/plugin.interface';
...
export function serializeSearchResultGranularSubComponents(list: FuseResult<AIFPlugin>[]): AIFPlugin[] {
    ...
    if (fuseResultItem.matches) {
      ...
      fuseResultItem.matches.forEach(match => {
          if (match.key === 'name' || match.key === 'description') {
            if (matchIndexesAlreadyAdded.indexOf(fuseResultItem.refIndex) === -1) {
              if(plugin.reportWidgets) matchedWidgets.push(...plugin.reportWidgets);
            ...
            if (matchIndexesAlreadyAdded.indexOf(match.refIndex) === -1) {
              if (match.key && match.key.startsWith('reportWidgets') && plugin.reportWidgets) {
                matchedWidgets.push({...plugin.reportWidgets[match.refIndex]});
              } else if (match.key && match.key.startsWith('inputBlocks') && plugin.inputBlocks) {
                matchedIBlocks.push({...plugin.inputBlocks[match.refIndex]});
              } else if (match.key && match.key.startsWith('algorithms') && plugin.algorithms) {
