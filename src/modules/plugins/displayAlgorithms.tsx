...
export default function DisplayAlgorithms (props: PropsWithChildren<DisplayAlgoritmsProps>) {
  ...
  function handleClickInputSchema(algo: Algorithm): MouseEventHandler<HTMLButtonElement> {
  ...
  function handleClickOutputSchema(algo: Algorithm): MouseEventHandler<HTMLButtonElement> {
  ...
  return (
    <div className={styles.componentDetails}>
      <div>
        {plugin.algorithms ? plugin.algorithms.map(algo => (
          <div className={clsx('pluginDetails-card', styles.componentCard)} key={`algo-${algo.gid}`}>
            ...
            <div className={styles.valueDisplay}>
              ...
              <div className={styles.value} style={{ color: '#991E66' }}>{algo.gid}</div>
            </div>
            <div className={styles.valueDisplay}>
            ...
            {algo.tags ? <div className={styles.valueDisplay}>
              <div className={styles.label}>Tags:</div>
              <div className={styles.value}>{algo.tags.join(', ')}</div>
            </div> : null}
            <div style={{ margin: '15px'}}>{children}</div>
            <div className={styles.footerBtnGroup}>
              <button className="aivBase-button aivBase-button--outlined aivBase-button--small" onClick={handleClickInputSchema(algo)}>
                Input Schema
              </button>
              <button className="aivBase-button aivBase-button--outlined aivBase-button--small" onClick={handleClickOutputSchema(algo)}>
                Output Schema
              </button>
            </div>
          </div>
