  ...
  return (
    <div style={{ width:"100%" }}>
              ...
              <div key={`check-${index}`} style={{ marginTop:'10px' }}>
                ...
                {check.processes.map((process, index2) => (
                  <div key={process.pid} style={{ width:"100%", display:"flex", backgroundColor:"#F3F0F5", padding:10, marginTop:5, marginBottom:5 }}>
                    <div style={{ fontWeight:600, width: "40px" }}>{index+1}.{index2+1}</div>
                    <div style={{ width:"100%" }}>
      ...
      <div style={{ backgroundColor:"#F3F0F5", padding:'10px' }}>
        <textarea rows="4" style={{ width:"100%", borderWidth:0, resize:"none" }} maxLength={256} value={data[`elaboration-${config.principle}`]} onChange={(e)=>onChangeData(`summary-justification-${config.principle}`,e.target.value)} />
      </div>
    </div>
  )
}