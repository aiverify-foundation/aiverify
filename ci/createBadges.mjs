...
if (badgeToCreate == 'lint') {
    ...
    if (numProblems.errors == 0 && numProblems.warnings == 0)
        color = 'brightgreen'
    else if (numProblems.errors > 0)
        color = 'red'
    else
        color = '#ffa500' // orange_2
    const lintBadge = {
        label: 'lint',
        message: numProblems.errors + " errors, " + numProblems.warnings + " warnings",
        color: color
    }
...
function testResults() {
    ...
    return { numPassedTests: testSummary.numPassedTests, numFailedTests: testSummary.numFailedTests }
}
...
function checkDependencies() {
    const text = fs.readFileSync('./npm-audit-report.md', { encoding: 'utf8', flag: 'r' })
...
function checkCopyleftLicenses() {
    const copyleftLic = [ 'GPL', 'LGPL', 'MPL', 'AGPL', 'EUPL', 'CCDL',
                          'EPL', 'CC-BY-SA', 'OSL', 'CPL' ]
    const text = fs.readFileSync('./license-report.txt', { encoding: 'utf8', flag: 'r' })
    const foundLic = []
