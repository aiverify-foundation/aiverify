...
class PluginTest:
    ...
    def _validate_metadata(self) -> Tuple[int, str]:
        ...
        if (
            metadata.name == "tensorflowpipeline"
            and metadata.description
            == "tensorflowpipeline supports detecting tensorflow pipeline"
...
@pytest.mark.parametrize(
    "file_path",
    [
        str(Path(__file__).parent / "user_defined_files/tensorflow_pipeline_files/"),
        str(Path(__file__).parent / "user_defined_files/tensorflow_pipeline_files.zip"),
        "https://github.com/aiverify-foundation/aiverify/raw/refs/heads/main/aiverify-test-engine/tests/io/user_defined_files/tensorflow_pipeline_files.zip",  # noqa: E501
    ],
)
def test_end_to_end_pipeline_plugin(file_path, request):
    ...
    expected_pipeline_algorithm = "fashionCustomClass.FashionMNISTTensorflow"
    expected_pipeline_plugin_type = PipelinePluginType.TENSORFLOW
