...
class Plugin(IData):
    """
    The Plugin(delimiterdata) class specifies methods on
    handling data formats.
    """

    # Some information on plugin
    _data: DelimiterMetadata = None
    _name: str = "delimiterdata"
    _description: str = "delimiterdata supports values separated by delimiters"
    _version: str = "0.9.0"
    ...
    def get_data(self) -> Any:
        ...
        Returns:
            Any: data
        """
        if isinstance(self._data, DelimiterMetadata) and self._data:
            return self._data.get_data()
        else:
            return None

    def get_delimiter_type(self) -> Union[DelimiterType, None]:
        """
        A method to return delimiter type

        Returns:
            Union[DelimiterType, None]: the delimiter type that the data is being delimited
        """
        if isinstance(self._data, DelimiterMetadata) and self._data:
            return self._data.get_delimiter_type()
        else:
            return None

    def set_data(self, data: DelimiterMetadata) -> None:
        """
        A method to set the data.

        Args:
            data (DelimiterMetadata): The data to replace the current data
        """
        if isinstance(data, DelimiterMetadata) and data:
            self._data = data
    ...
    def is_supported(self) -> bool:
        ...
        """
        return isinstance(self._data, DelimiterMetadata)
    ...
    def convert_to_dict(self) -> Dict:
        ...
        """
        if isinstance(self._data, DelimiterMetadata) and self._data:
            data_path = self._data.get_data_path()
            delimiter_char = self._data.get_delimiter_char()
            return {"data_path": data_path, "delimiter_type": delimiter_char}
        else:
            return dict()
