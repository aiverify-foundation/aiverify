...
class Plugin(IPipeline):
    """
    The Plugin(tensorflowpipeline) class specifies methods on
    ...
    _name: str = "tensorflowpipeline"
    _description: str = "tensorflowpipeline supports detecting tensorflow pipeline"
    _version: str = "0.9.0"
    _metadata: PluginMetadata = PluginMetadata(_name, _description, _version)
    _plugin_type: PluginType = PluginType.PIPELINE
    _pipeline_plugin_type: PipelinePluginType = PipelinePluginType.TENSORFLOW
    ...
    def _identify_pipeline_algorithm(self, pipeline: Any) -> Tuple[bool, str]:
        ...
        Returns:
            ...
            pipeline name
        """
        is_success = isinstance(pipeline, tf.keras.Model)
        pipeline_algorithm = f"{type(pipeline).__module__}.{type(pipeline).__name__}"
