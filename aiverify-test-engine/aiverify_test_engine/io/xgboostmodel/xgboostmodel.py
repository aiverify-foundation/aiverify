...
class Plugin(IModel):
    """
    The Plugin(xgboostmodel) class specifies methods on
    ...
    _name: str = "xgboostmodel"
    _description: str = "xgboostmodel supports detecting xgboost models"
    _version: str = "0.9.0"
    _metadata: PluginMetadata = PluginMetadata(_name, _description, _version)
    _plugin_type: PluginType = PluginType.MODEL
    _model_plugin_type: ModelPluginType = ModelPluginType.XGBOOST
    ...
    def predict_proba(self, data: Any, *args) -> Any:
        ...
        try:
            if self._model_algorithm == "xgboost.core.Booster":
                return self._model.predict_proba(xgboost.DMatrix(data))
            else:
