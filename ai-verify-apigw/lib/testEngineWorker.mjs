...
const processEvent = (key) => {
  ...
  redis2.hGetAll(key).then(async values => {
    ...
    if ((values.type === 'TaskResponse')) {
      ...
      // console.debug("testId", testId, "reportId", reportId)
      // if (values.errorMessages) {
      //   // temp solution until toolbox fix invalid JSON
      //   values.errorMessages = values.errorMessages.replaceAll("'",'"');
      // }
      let doc;
      try {
      ...
      if (values.status === 'Success' || values.status === 'Error' || values.status === 'Cancelled') {
  ...
  redis2.hGetAll(key).then(async values => {
    ...
    if ((values.serviceType === 'validateDataset')) {
        ...
        try {
            ...
            if (doc.status !== 'Cancelled') {
              if (values.status === 'done') {
                ...
                if (values.validationResult === 'valid') {
                  ...
                  if (values.columns && values.dataFormat && values.serializedBy && values.numRows && values.numCols && values.logFile) {
          ...
          console.error("Error updating dataset validation result for ", datasetId,", error: ", err);
          redis2.del(key); // delete key
          return;
        }
    }
      ...
      try {
        ...
        console.error("Error updating model validation result for ", modelFileId,", error: ", err);
        redis2.del(key);
        return;
      }
    }
