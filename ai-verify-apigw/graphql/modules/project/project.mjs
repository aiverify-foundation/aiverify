...
const resolvers = {
    Query: {
        ...
        projects: (parent) => {
            ...
            return new Promise((resolve, reject) => {
                ProjectModel.find({ __t: 'ProjectModel' }).populate("report").populate("template").then(docs => {
                    // console.debug("docs", docs);
                    const results = docs.map( doc => {
                        if (doc.report)
                            doc.report.projectID = doc._id;
                        return doc;
                    })
                    resolve(results)
                }).catch(err => {
        ...
        project: (parent, {id}) => {
            return new Promise((resolve, reject) => {
                ProjectModel.findById(id).populate("report").populate("template").then(doc => {
                    if (!doc)
        ...
        report: (parent, {projectID}) => {
            return new Promise(async (resolve, reject) => {
                const report = await ReportModel.findOne({ project: projectID }).populate("projectSnapshot")
                if (report) {
                    ...
                    await report.populate("projectSnapshot.modelAndDatasets.groundTruthDataset")
                    if (report.tests) {
    ...
    Mutation: {
        ...
        createProject: (parent, {project}) => {
            ...
            return new Promise((resolve, reject) => {
                ProjectModel.create(project).then((doc) => {
        ...
        createProjectFromTemplate: (parent, {project, templateId}) => {
            ...
            return new Promise(async (resolve, reject) => {
                ProjectTemplateModel.findById(templateId).then(template => {
                    ...
                    ProjectModel.create(newProject).then((doc) => {
                        // console.debug("doc", doc);
                        // doc.save().then(())
                        resolve(doc);
                    }).catch(err => {
        ...
        deleteProject: (parent, {id}) => {
            ...
            return new Promise((resolve, reject) => {
                ProjectModel.findByIdAndDelete(id).then(result => {
                    if (!result)
                     return reject("Invalid ID")
                    resolve(id);
                }).catch(err => {
                ...
                try {
                    ...
                    if (modelAndDatasets) {
                        if (modelAndDatasets.modelId) {
                            modelAndDatasets.model = mongoose.Types.ObjectId(modelAndDatasets.modelId);
                        ...
                        if (modelAndDatasets.groundTruthDatasetId) {
                            modelAndDatasets.groundTruthDataset = mongoose.Types.ObjectId(modelAndDatasets.groundTruthDatasetId);
        ...
        cloneProject: (parent, {id}) => {
            return new Promise((resolve, reject) => {
                ProjectModel.findById(id).then(doc => {
                    if (!doc)
                        return reject("Invalid ID")
        ...
        saveProjectAsTemplate: (parent, {projectId, templateInfo}) => {
            return new Promise((resolve, reject) => {
                ProjectModel.findById(projectId).then(doc => {
                    if (!doc)
                        return reject("Invalid ID")
                    ...
                    delete newdoc.__t;
                    newdoc.isNew = true;
                    ProjectTemplateModel.create(newdoc).then((doc) => {
                        // console.debug("doc", doc);
                        // doc.save().then(())
                        resolve(doc);
                    }).catch(err => {
        ...
        generateReport: (parent, {projectID, algorithms}) => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                if (!proj)
                    return reject("Invalid project ID");
                // console.log("proj", proj)
                if (proj.report && (proj.report.status === "GeneratingReport" || proj.report.status === "RunningTests"))
                ...
                if (algorithms && algorithms.length > 0 && proj.testInformationData) {
                    ...
                    for (const gid of algorithms) {
                        ...
                        if (Object.keys(inputSchema) === 0) { // make sure gid exists
                            console.log("Invalid GID");
                            continue;
                        }
                        let test = proj.testInformationData.find(e => e.algorithmGID === gid);
                        if (!test) {
                            // continue;
                ...
                if (needToRunTests && (!modelAndDatasets || !modelAndDatasets.model || !modelAndDatasets.testDataset)) {
                    return reject("No model and test dataset defined");
                }
                reportObj.status = needToRunTests ? "RunningTests" : "GeneratingReport";
                if (proj.report) { // has existing report object
                    const reportId = proj.report._id;
                    const doc = await ReportModel.findByIdAndUpdate(reportId, reportObj, { new:true })
        ...
        cancelTestRuns: (parent, {projectID, algorithms}) => {
            ...
            return new Promise(async (resolve, reject) => {
                ...
                if (!report)
                    return reject("Report not found");
                if (report.status != "GeneratingReport" && report.status != "RunningTests")
