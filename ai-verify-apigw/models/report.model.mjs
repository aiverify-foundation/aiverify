...
const errorMessageSchema = new Schema({
  code: String,
  severity: { type: String, enum: ['information', 'warning', 'critical' ], required: true },
...
const testEngineTaskSchema = new Schema({
	...
	status: { type: String, required: true, enum: ["Pending", "Running", "Cancelled", "Success", "Error" ] },
...
const schema = new Schema({
  ...
  status: { type: String, required: true, enum: ["NoReport","RunningTests","GeneratingReport","ReportGenerated","ReportError"], default:"NoReport" },
  timeStart: { type: Date },
  timeTaken: { type: Number }, // in seconds
