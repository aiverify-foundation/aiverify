...
class TestResultModel(BaseORMModel):
    ...
    def __repr__(self) -> str:
        artifacts = ", ".join([artifact.filepath for artifact in self.artifacts])
        return f"TestResultModel(id={self.id}, gid={self.gid}, cid={self.cid}, version={self.version}, user={self.user}, start_time={self.start_time}, time_taken={self.time_taken}, test_arguments={self.test_arguments.decode('utf-8')}, output={self.output.decode('utf-8')}, artifacts={artifacts})"
