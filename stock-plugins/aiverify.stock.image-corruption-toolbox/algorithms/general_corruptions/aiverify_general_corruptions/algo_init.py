...
class AlgoInit:
    ...
    def __init__(
        ...
        if not user_corruptions or "all" in user_corruptions:
            ...
            self._user_defined_params["corruptions"] = list(general.CORRUPTION_FN)
        else:
            # Filter corruption functions based on user input, make sure algorithm names are in correct format & order
            self._user_defined_params["corruptions"] = [name for name in general.CORRUPTION_FN if name.lower() in user_corruptions]  # fmt: skip
