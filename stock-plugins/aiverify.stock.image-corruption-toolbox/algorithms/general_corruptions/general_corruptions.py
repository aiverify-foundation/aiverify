...
class Plugin(IAlgorithm):
    ...
    The Plugin(General Corruptions) class specifies methods in generating results for algorithm
    """

    # Some information on plugin
    _name: str = "General Corruptions"
    _description: str = "Robustness with general corruptions"
    _version: str = "0.9.0"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._save_path = self._base_path.parents[1] / "widgets" / "general_images"
    ...
    def _gaussian_noise(self, img: np.ndarray, severity: int = 1) -> np.ndarray:
        ...
        out = np.clip(
            img + np.random.normal(size=img.shape, scale=severity_constant), 0, 1
        )
    ...
    def _snp_noise(self, img: np.ndarray, severity: int = 1) -> np.ndarray:
        ...
        flipped = np.random.choice(
            [True, False], size=img.shape, p=[amount, 1 - amount]
        )
