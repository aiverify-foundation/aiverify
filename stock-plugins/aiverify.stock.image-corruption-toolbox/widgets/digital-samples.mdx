...
export const cid = "aiverify_digital_corruptions";

export const DigitalSamples = ({ props }) => {
  ...
  const corruptionTypes = getCorruptionTypes(resultsData, "Digital");

  if (corruptionTypes.length === 0) {
    return <div>No digital corruption data found</div>;
  }
  ...
  corruptionTypes.forEach((corruptionFunction) => {
    ...
    const formattedCorruptionName = formatCorruptionName(corruptionFunction, "Digital");
  ...
  if (corruptionResults.length === 0) {
    return <div>Could not process any digital corruption types</div>;
  }
