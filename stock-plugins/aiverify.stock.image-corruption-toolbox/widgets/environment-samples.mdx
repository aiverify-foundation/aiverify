...
export const cid = "aiverify_environment_corruptions";

export const EnvironmentSamples = ({ props }) => {
  ...
  const corruptionTypes = getCorruptionTypes(resultsData, "Environment");

  if (corruptionTypes.length === 0) {
    return <div>No environment corruption data found</div>;
  }
  ...
  corruptionTypes.forEach((corruptionFunction) => {
    ...
    const formattedCorruptionName = formatCorruptionName(corruptionFunction, "Environment");
  ...
  if (corruptionResults.length === 0) {
    return <div>Could not process any environment corruption types</div>;
  }
...
<EnvironmentSamples props={props} />
