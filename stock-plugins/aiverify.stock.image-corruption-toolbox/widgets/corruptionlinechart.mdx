import React, { useEffect, useState } from 'react';
import { LineChart } from 'aiverify-shared-library/charts'

export const CorruptionLineChart = ({ data, corruption_function, width, height  }) => {
  const [mydata, setmydata] = useState({
    datas: [],
    dataKeys: [],
  });

  const wrapperStyle = {
    fontSize: "12px"
  }

  useEffect(() => {
    if (!data)
      return;

    let compiledData = []
    let groups = {}

    for (let sev = 0; sev < 6; sev++) {
      let obj = {
        "name": `Severity ${sev}`
      }
      for (let cf = 0; cf < data.length; cf++) {
        const result = data[cf]["accuracy"]
        const function_name = data[cf]["corruption_function"].replace(/_/g, " ")
        obj[function_name] = result[`severity${sev}`]*100
        groups[function_name] = 1
      }
      compiledData.push(obj)
    }

    setmydata({
      datas: compiledData,
      dataKeys: Object.keys(groups).map(grp => ({ dataKey: grp })),
    });
  }, [data])

  return (
    <div style={{ width, height, padding: 10, display: "flex", flexDirection: "column" }}>
      <h4 style={{ textAlign: "center" }}>{corruption_function}</h4>
      <div style={{ height: "calc(100% - 40px)" }}>
        <LineChart
          data={mydata.datas}
          xAxisDataKey="name"
          lines={mydata.dataKeys}
          yAxisProps={{ label: { value: 'Accuracy', angle: -90, position: 'insideLeft' } }}
        />
      </div>
    </div>
  )
}
