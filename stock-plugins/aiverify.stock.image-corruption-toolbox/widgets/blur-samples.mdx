...
import { getBlurTypes, formatCorruptionName } from "./corruption-utils";
...
export const BlurSamples = ({ props }) => {
  ...
  const blurTypes = getBlurTypes(resultsData);

  if (blurTypes.length === 0) {
    return <div>No blur corruption data found</div>;
  }

  const corruptionResults = [];

  blurTypes.forEach((corruptionFunction) => {
    ...
    const formattedCorruptionName = formatCorruptionName(corruptionFunction);
