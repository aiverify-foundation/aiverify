export const config = {
  ...
  "summaryNotYes": "Company should review if the current communication mechanisms in place are sufficient to enable those using and/or affected by the AI system to understand how their data is collected and used, the intended use and limitations of the AI system, and that they are updated when incidents occur.",
  "recommendation": "Company can consider consulting the users of or individuals affected by the AI system to find out if the current level of information provided to them and the means for feedback to the Company are adequate, and if not, to address the information gap accordingly."
}