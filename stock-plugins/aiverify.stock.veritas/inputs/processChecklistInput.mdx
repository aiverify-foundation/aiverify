            ...
            {section.checklist.map((check, index) => (
              <div key={`check-${index}`} style={{ marginTop:'10px' }}>
                ...
                {check.processes
                  ...
                  .map((process, index2) => (
                    <div key={process.pid} style={{ width:"100%", display:"flex", backgroundColor:"#E5F1F9", color: "black", padding:10, marginTop:5, marginBottom:5 }}>
                      <div style={{ fontWeight:600, width: "50px" }}>{process.pid}</div>
                      <div style={{ width:"100%" }}>
                        <div dangerouslySetInnerHTML={{ __html:process.process }}></div>
                        <div style={{ display:"flex", marginTop:5 }}>
                        </div>
                        <h4 style={{ padding:0, margin:"5px 0 0 0", color: "black" }}>Completed</h4>
                        ...
                        <h4 style={{ padding:0, margin:"5px 0 0 0", color: "black" }}>Elaboration</h4>
