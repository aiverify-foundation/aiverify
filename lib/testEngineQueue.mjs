...
function updateReport(reportId, update) {
  ...
  return TestRun.findByIdAndUpdate(
    { _id:reportId },
    { "$set": update },
  );
}
...
export const queueTests = async (report, modelAndDatasets) => {
  ...
  for (let test of report.tests) {
    ...
    if (modelAndDatasets && modelAndDatasets.testDataset) {
      task.testDataset = modelAndDatasets.testDataset.filePath;
    }
    if (modelAndDatasets && modelAndDatasets.model) {
      task.modelFile = modelAndDatasets.model.filePath;
    }
    if (modelAndDatasets && modelAndDatasets.groundTruthDataset) {
...
export const cancelTestRun = async (report, test) => {
    ...
    if (test.status === 'Pending') {
      ...
      await redis.publish(redisCancelTopic, taskId);
      await Promise.all([
        redis.hSet(taskId, 'status', 'Cancelled'),
      ])
...
export const queueDataset = async (dataset) => {
  ...
  console.debug(`XADD ${redisServiceQueue} * validateDataset '${JSON.stringify(service)}'`)
  await redis.xAdd(redisServiceQueue, "*", { "validateDataset": JSON.stringify(service) })
}
...
export const queueModel = async (modelFile) => {
  ...
  console.debug(`XADD ${redisServiceQueue} * validateModel '${JSON.stringify(service)}'`)
  await redis.xAdd(redisServiceQueue, "*", { "validateModel": JSON.stringify(service) })
}
