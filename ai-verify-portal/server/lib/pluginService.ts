...
const python_script_checker = path.join(
  process.cwd(),
  'server/scripts/syntax_checker.py'
);
...
export async function scanPluginDirectory(
  pdir: string,
  cached: AIFPluginCache | null = null
): Promise<AIFPlugin> {
  console.log('scanPluginDirectory', pdir);
  // read in the plugin meta file
  const pluginMeta = readJSONFromFile(
    path.join(pdir, pluginMetaFile)
  ) as AIFPlugin;
  ...
  if (fs.existsSync(widgets_subdir)) {
    ...
    for (const metaFile of metaFiles) {
      ...
      widget.mdxPath = path.relative(
        PLUGIN_DIR,
        path.join(widgets_subdir, `${widget.cid}.mdx`)
      );
      ...
      if (widget.dependencies) {
        for (const dep of widget.dependencies) {
          dep.gid =
            dep.gid && dep.gid.length > 0
              ? `${dep.gid}:${dep.cid}`
              : `${pluginMeta.gid}:${dep.cid}`;
        }
      }
  ...
  if (fs.existsSync(inputs_subdir)) {
    ...
    for (const metaFile of metaFiles) {
      ...
      widget.version = pluginMeta.version;
      widget.pluginGID = pluginMeta.gid;
      if (!widget.width) widget.width = 'md'; // set default width to md
      // set the path to the MDX
      widget.mdxPath = path.relative(
        PLUGIN_DIR,
        path.join(inputs_subdir, `${widget.cid}.mdx`)
      );
      widget.summaryPath = path.relative(
        PLUGIN_DIR,
        path.join(inputs_subdir, `${widget.cid}.summary.mdx`)
      );
  ...
  if (fs.existsSync(algo_subdir)) {
    ...
    for (const cid of algodirs) {
      ...
      if (!fs.lstatSync(mydir).isDirectory()) continue;
      ...
      const requirements = readRequirementsFile(
        path.join(mydir, 'requirements.txt')
      );
      // read input and output schema
      const inputSchema = readJSONFromFile(
        path.join(mydir, 'input.schema.json'),
        null
      );
      const outputSchema = readJSONFromFile(
        path.join(mydir, 'output.schema.json'),
        null
      );

      txn = txn
        .hSet(key, 'data', JSON.stringify(algo))
...
function validatePythonScript(scriptPath: string): Promise<void> {
  return new Promise((resolve, reject) => {
    if (!fs.existsSync(scriptPath)) return reject('Script does not exists');

    try {
      exec(
        `${python_cmd} ${python_script_checker} "${scriptPath}"`,
        (error) => {
          if (error) return reject('Invalid python script');
          else return resolve();
        }
      );
    } catch (e) {
...
async function validateMDX(scriptPath: string): Promise<void> {
  await bundleMDX({
    ...
    esbuildOptions: (options) => {
      ...
      options.external = ['ai-verify-shared-library/*'];
      return options;
    },
  });
