 ...
 * @returns Promise with Id of new project on success
 */
type CreateProjectFromTemplateFunction = (
  project: Partial<Project>,
  id: string
) => Promise<Partial<Project>>;
export const useCreateProjectFromTemplate =
  (): CreateProjectFromTemplateFunction => {
 ...
 * @returns Promise with new cloned project
 */
type saveProjectAsTemplateFunction = (
  projectId: string,
  templateInfo: ProjectInformation
 ...
 * @returns
 */
export const useSubscribeReportStatusUpdated = (
  projectID: string
): [
  { reportStatusUpdated: Report } | undefined,
  boolean,
  ApolloError | undefined
] => {
  const { data, loading, error } = useSubscription<
    { reportStatusUpdated: Report } | undefined
  >(REPORT_STATUS_UPDATE_SUBSCRIPTION, { variables: { projectID } });
  return [data, loading, error];
};
