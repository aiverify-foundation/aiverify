      ...
      <div className="layoutContentArea">
        <Container maxWidth={false} className="scrollContainer">
          <Container maxWidth="xl" className="mainContainer">
            <Paper sx={{ p: 2, m: 2, display: 'flex', flexDirection: 'column', height: '80vh' }} >
                <Box sx={{ height: '5%', display: 'flex', alignItems:'center' }}>
                  <Button startIcon={<NavigateBeforeIcon />} color="secondary" sx={{ mr: 1 }}
                    onClick={() => router.push('/home')} data-testid="assets-back-button"
                  />
                ...
                <Box sx={{ p: 10, height: '95%' }}>
                  <Grid container spacing={2} sx={{ mb: 1 }}>
                    <Grid xs={4}>
                      <div className={styles.newButton} onClick={() => router.push('/assets/newDataset')} data-testid="add-new-dataset-button">
                        <div style={{display: 'flex', flexDirection: 'row', gap: '20px', height: '30px', alignItems: 'center'}}>
                          <InsertDriveFileIcon></InsertDriveFileIcon>
                          <div style={{fontSize: '18px', fontWeight:'500'}}>New Dataset</div>
                        </div>
                        <AddIcon></AddIcon>
                      </div>
                    </Grid>
                    <Grid xs={4}>
                      <div className={styles.newButton} onClick={() => router.push('/assets/newModel')} data-testid="add-new-model-button">
                        <div style={{display: 'flex', flexDirection: 'row', gap: '20px', height: '30px', alignItems: 'center'}}>
                          <InsertDriveFileIcon></InsertDriveFileIcon>
                          <div style={{fontSize: '18px', fontWeight:'500'}}>New AI Model</div>
                        </div>
                        <AddIcon></AddIcon>
                      </div>
                    </Grid>
                  </Grid>
                  <Typography variant="h6" sx={{ pb: 2, fontWeight: 'bold', marginTop:'50px' }}>
