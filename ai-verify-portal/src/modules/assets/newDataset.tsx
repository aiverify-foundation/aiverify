...
function DatasetsPicking({
  updateValidatingState,
  updateFileList,
  ...
  const pickFolders = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!event.target) return;
      ...
      if (isLargeFile == true) {
        messages.push(
          'Maximum file upload size is 4GB. Please upload smaller files.'
        );
  ...
  function pickFiles(files: FileList) {
    ...
    if (!files) return;
      ...
      if (fileList.length > 10) {
        ...
        if (folderSelected) {
          messages.push(
            "Folder upload not supported for drag and drop. For image datasets, please click on 'UPLOAD FOLDER' to select folder."
          );
          setAlertTitle('File selection error');
          setAlertMessages(messages);
        } else {
  ...
  useEffect(() => {
    ...
    const uploadDatasetFiles = async (signal: AbortSignal) => {
      ...
      if (foldersToUpload.length != 0) {
        formData.append(
          'myFolder',
          filesToUpload[0].webkitRelativePath.split('/').slice(0, -1).join('/')
        );
      }
    ...
    if (tryUpload) {
      ...
      if (foldersToUpload.length == 0 && filesToUpload.length > 10) {
        ...
        filenames.forEach((str) =>
          alreadySeen[str] ? (isDuplicate = true) : (alreadySeen[str] = true)
        );
        if (isDuplicate) {
  ...
  const Dropbox = () => (
    <Box
      ...
      data-testid="upload-file-dropbox">
      <UploadIcon />
      <Typography sx={{ fontSize: 20 }} color="primary">
        Drag &#38; Drop or <b>Click to Browse</b>
      </Typography>
      <Typography sx={{ fontSize: 14 }}>Maximum 10 files per upload</Typography>
    </Box>
  );
  ...
  return (
    <div
      ...
      <div
        ...
        <Box sx={{ flexGrow: 2, maxWidth: '580px' }}>
          ...
          <Box
            ...
            <Typography sx={{ pl: 2, fontSize: 14 }}>
              ...
              <b>Data Format:</b> Pandas, Delimiter-separated Values (comma,
              tab, semicolon, pipe, space, colon), Image (jpeg, jpg, png)
              <br />
        ...
        <Box
          sx={{ flexGrow: 3, pl: 5, display: 'flex', flexDirection: 'column' }}>
          <FileUploader
            multiple={true}
          ...
          <Box
            ...
            <Typography sx={{ fontSize: 14 }}>For image datasets,</Typography>
            <input
              disabled={
                isUploading ||
      ...
      <div style={{ marginLeft: '3%', width: '90%', flexGrow: 2 }}>
                    ...
                    <ListItem key={file.name}>
                      <ListItemAvatar>
                        <IconButton
                          disabled={isUploading}
                          onClick={() => {
                            unpickDatasetFile(file);
                          }}
                          data-testid="unpick-file">
                          <ClearIcon />
                        </IconButton>
                      </ListItemAvatar>
                      <ListItemText
                        data-testid="dataset-upload-list"
                        primary={file.name}
        ...
        <Container
          ...
          <Button
            variant="contained"
            component="label"
            disabled={filesToUpload.length == 0 || isUploading == true}
            onClick={() => setTryUpload(true)}
            data-testid="upload-datasets-button">
...
function DatasetsValidating({
  filesValidating,
  updateFile,
  onBackClick,
}: DatasetsValidatingProps) {
  ...
  const [datasetsValidating, setDatasetsValidating] = useState<
    (Dataset & { _id?: string })[]
  >([]);
  ...
  useEffect(() => {
    const stillValidating = datasetsValidating.find(
      (e: Dataset) => e.status.toString() === FileStatus.PENDING
    );
    if (stillValidating) {
        ...
        <div
          ...
          <Box
            ...
            <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
              {!isFailed &&
                datasetsValidating &&
                datasetsValidating.map((datasetValidating) => (
                  <div
                    key={datasetValidating._id}
                    onClick={() => onSetFocus(datasetValidating)}
                    style={{
                      backgroundColor:
                        datasetValidating._id == focus?._id ? '#F3F0F5' : '',
                    }}>
                    <ListItemButton>
                      <ListItemAvatar>
                      ...
                      <ListItemText
                        primary={datasetValidating.name}
                        secondary={datasetValidating.size}
                        sx={{ width: 360 }}
                      />
                      <ListItemIcon>
                        <IconButton
                          ...
                          onClick={() =>
                            setCancelled(
                              datasetValidating._id as string,
                              datasetValidating
                            )
                          }
                  ...
                  <div
                    ...
                    <ListItemButton>
                      ...
                      {datasetValidating.status == FileStatus.PENDING && (
                        ...
                        <ListItemText
                          primary={datasetValidating.name}
                          secondary={`File upload failed. Please try again.`}
                        />
                      )}
                      {datasetValidating.status == FileStatus.VALID && (
                        <ListItemText
                      ...
                      {datasetValidating.status == FileStatus.ERROR && (
                        <ListItemText
                          primary={datasetValidating.name}
                          secondary={`File upload failed. Please upload again.`}
                        />
                      )}
                      {datasetValidating.status == FileStatus.CANCELLED && (
                        <ListItemText
                          primary={datasetValidating.name}
                          secondary={`File validation cancelled. Please upload again`}
                        />
                      )}
                    </ListItemButton>
                    <hr
            ...
            <Box
              ...
              <Typography sx={{ fontSize: 14 }}>
                ...
                {focus.status == FileStatus.VALID ? (
                  <Typography sx={{ fontSize: 14 }}>
                    <b>Shape:</b> {focus.numRows} Rows | {focus.numCols} Cols{' '}
                    <br />
              ...
              {focus.status == FileStatus.VALID && (
                <TableContainer
                  ...
                  <Table sx={{ width: '100%' }}>
                    ...
                    <TableBody>
                      {focus.dataColumns &&
                        focus.dataColumns.map((dataColumn) => (
                          <TableRow
                            key={dataColumn.name}
                            sx={{
        ...
        <Dialog
          ...
          <DialogContent>
            ...
            <Typography sx={{ fontSize: 14, lineHeight: '150%' }}>
              ...
              <p>
                <b>Shape:</b> {edit.numRows} Rows | {edit.numCols} Cols
              </p>
              <p>
                <b>Serializer</b> {edit.serializer}
              </p>
              <p>
                <b>Data Format</b> {edit.dataFormat}
              </p>
            </Typography>
            ...
            {edit.status == FileStatus.VALID && (
              <TableContainer
                ...
                <Table sx={{ width: '100%' }}>
                  ...
                  <TableBody>
                    {edit.dataColumns &&
                      edit.dataColumns.map((dataColumn) => (
                        <TableRow
                          key={dataColumn.name}
                          sx={{
                            '&:last-child td, &:last-child th': { border: 0 },
          ...
          <DialogActions>
            ...
            <Button onClick={() => onUpdateDataset(edit._id, edit)}>
              Save & Close
            </Button>
          </DialogActions>
        </Dialog>
...
export default function NewDatasetModule({
  ...
  const [filesValidating, setFilesValidating] = useState<
    (Dataset & { _id?: string })[]
  >([]); //👈 Sort out id at Dataset type definition level.
  ...
  useSubscription(VALIDATE_DATASET_UPDATED, {
    ...
    onData: (payload) => {
      ...
      if (
        data == undefined ||
        data.data.validateDatasetStatusUpdated == undefined
      )
        return;
  ...
  if (withoutLayoutContainer) {
    return (
      <Paper
        ...
        <Box
          sx={{ mb: 2, height: '5%', display: 'flex', alignItems: 'center' }}>
          <Button
        ...
        {isValidating == false && (
          <DatasetsPicking
            updateValidatingState={updateValidatingState}
            updateFileList={updateFileList}></DatasetsPicking>
        )}
        {isValidating == true && (
          <DatasetsValidating
            filesValidating={filesValidating}
            updateFile={updateFile}
            onBackClick={handleBackIconClick}></DatasetsValidating>
        )}
      </Paper>
    );
  }
            ...
            <Paper
              ...
              <Box
                ...
                <Button
                  ...
                  data-testid="newdataset-back-button"
                />
                <Typography
                  variant="h6"
                  sx={{ pl: 2, flexGrow: 1, fontWeight: 'bold' }}>
              ...
              {isValidating == true && (
                <DatasetsValidating
                  filesValidating={filesValidating}
                  updateFile={updateFile}></DatasetsValidating>
              )}
            </Paper>
          </Container>
        </Container>
