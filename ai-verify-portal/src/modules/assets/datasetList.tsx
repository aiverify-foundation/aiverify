...
export default function DatasetListComponent({
  ...
  useEffect(() => {
    if (data) {
      ...
      const filteredFiles = ar.reduce<Dataset[]>((collection, current) => {
        if (
          current.type &&
          current.type.toString() == DatasetFilters.FILE &&
          filters.indexOf(DatasetFilters.FILE) > -1
        ) {
          return [...collection, current];
        }
        if (
          current.type &&
          current.type.toString() == DatasetFilters.FOLDER &&
          filters.indexOf(DatasetFilters.FOLDER) > -1
        ) {
          return [...collection, current];
        }
        return collection;
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      if (focus) {
        const focusedItem = data?.datasets.find(
          (e: Dataset) => e.id?.toString() === focus.id?.toString()
        );
        setFocus(focusedItem);
        setEdit(focusedItem);
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params: GridRenderCellParams) => {
        if (params.row.status.toString() === 'Valid') {
          return (
            <Typography sx={{ fontSize: 16 }}>{params.value} Rows</Typography>
          );
        } else {
          return (
            <Chip
    ...
    },
    {
      ...
      renderCell: (params: GridRenderCellParams) => {
        if (params.row.status.toString() === 'Valid') {
          return (
            <Typography sx={{ fontSize: 16 }}>{params.value} Cols</Typography>
          );
        } else {
          return (
            <Chip
      ...
      <GridToolbarContainer
        ...
        <div style={{ flex: '1 1', minWidth: '250px' }}>
          ...
          <Chip
            ...
            variant={
              filters.indexOf(DatasetFilters.FILE) > -1 ? 'filled' : 'outlined'
            }
            color="default"
            sx={{ mr: '5px' }}
            clickable
    ...
    <React.Fragment>
          ...
          <AlertBox.Footer>
            <div
              ...
              <Button
                ...
                sx={{ m: 2 }}
                onClick={() => {
                  onDeleteDataset(selectionModel);
                }}>
                Delete Files
              </Button>
            </div>
      ...
      <Box
        ...
        <DataGrid
          ...
          localeText={{
            noRowsLabel:
              'No dataset files uploaded. Click "NEW DATASET +" to upload new dataset files.',
          }}
        />
        {focus && edit && (
          <Box
            ...
            <Typography sx={{ fontSize: 14 }}>
              ...
              <br />
              <b>Size:</b> {focus.size ? focus.size : '-'}
              <br />
            </Typography>
            ...
            {showSelectDatasetBtn && focus.status == FocusStatus.VALID && (
              <Button
                onClick={handleSelectDatasetBtnClick}
                variant="contained"
                style={{ margin: 10 }}>
                Use Dataset
              </Button>
      ...
      {edit && (
        <Dialog
          ...
          <DialogActions>
            ...
            <Button onClick={() => onUpdateDataset(edit.id as string, edit)}>
              Save & Close
            </Button>
          </DialogActions>
        </Dialog>
