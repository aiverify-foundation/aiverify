...
export default function ModelListComponent({ 
    ...
    useEffect(() => {
        if (data) {
            ...
            const filteredFiles = ar.reduce<ModelFile[]>((collection, current) => {
                if (current.modelType && current.modelType.toString() == ModelFilters.CLASSIFICATION && filters.indexOf(ModelFilters.CLASSIFICATION) > -1) {
                    return [...collection, current];
                }
                if (current.modelType && current.modelType.toString() == ModelFilters.REGRESSION && filters.indexOf(ModelFilters.REGRESSION) > -1) {
                    return [...collection, current];
                }
                if (current.type && current.type == ModelFilters.FILE && filters.indexOf(ModelFilters.FILE) > -1) {
                    return [...collection, current];
                }
                if (current.type && current.type == ModelFilters.FOLDER && filters.indexOf(ModelFilters.FOLDER) > -1) {
                    return [...collection, current];
                }
                if (current.type && current.type == ModelFilters.PIPELINE && filters.indexOf(ModelFilters.PIPELINE) > -1) {
                    return [...collection, current];
                }
                return collection;
            }, []);
    ...
    useEffect(() => {
        ...
        if (data) {
            ...
            if (focus) {
                const focusedItem = data?.modelFiles.find((e: ModelFile) => e.id?.toString() === focus.id?.toString());
                setFocus(focusedItem);
                setEdit(focusedItem);
            }
        } 
        ...
        { 
            ...
            renderCell: (params: GridRenderCellParams) => {
                if ( params.value.toString() === "Folder" ) {
                    ...
                    return (<div><Tooltip title="Pipeline">
            ...
            renderCell: (params: GridRenderCellParams) => {
                if ( params.row.status.toString() === "Valid" ) {
                    if ( params.value ) {
                        ...
                        return (
                            <Chip icon={<CancelIcon/>} label="Unspecified" variant="outlined" color='warning' sx={{ width: { md: '100px', lg: 'auto' }}}/>
                        )
                    }
                } else {
                    return (
                        <Chip icon={<CancelIcon/>} label="Model Invalid" variant="outlined" color='error' sx={{ width: { md: '100px', lg: 'auto' }}}/>
                    )
                }
            }, 
        },
    ...
    return (
        <React.Fragment>
                ...
                <AlertBox
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', width: '600px'}}>
                        ...
                        <Button variant="contained" component="label" sx={{m:2}}
                            onClick={() => {onDeleteModelFile(selectionModel)}}>
                            Delete Files
                        </Button>
                    </div>
                    </AlertBox.Footer>
            ...
            <Box justifyContent="center" sx={{
                ...
                <DataGrid
                    sx={{p:2, flex: 2, borderRadius: 3, m:1, ...gridStyles,
                    ...
                    localeText={{
                        noRowsLabel:'No models loaded. Click "NEW MODEL +" to load new models.',
                    }}
                />
                {focus && edit && (
                    <Box sx={{ overflow: 'auto', maxHeight: '500px', p:2, m:1, display: 'flex', flexDirection: 'column', flex: 1, border: '1px solid #C8C8C8', borderRadius: 3}}>
                        ...
                        <Button onClick={handleModelBtnClick} variant="contained" style={{margin:10}}>Use Model</Button>
                        )}
                    </Box>
                )}
            </Box>
            {edit && (
                ...
                >
                    ...
                    <DialogActions>
                        ...
                        <Button onClick={() => onUpdateModel(edit.id, edit)}>Save & Close</Button>
                    </DialogActions>
                </Dialog>
            )}
        </React.Fragment>
