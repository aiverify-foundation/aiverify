...
function ModelsPicking({
  ...
  function pickFiles(files: FileList) {
    ...
    if (!files) return;
      ...
      if (fileList.length > 10) {
        ...
        if (folderSelected) {
          messages.push(
            "Folder upload not supported for drag and drop. For TensorFlow models, please click on 'UPLOAD FOLDER' to select folder"
          );
          setAlertTitle('File selection error');
          setAlertMessages(messages);
        } else {
  ...
  useEffect(() => {
    ...
    const uploadModelFiles = async (signal: AbortSignal) => {
      ...
      for (const { index, value } of filesToUpload.map((value, index) => ({
        index,
        value,
      }))) {
      ...
      axios
        .post('/api/upload/model', formData, {
          signal: signal,
        ...
        <Box sx={{ flexGrow: 2 }}>
          ...
          <Box
            ...
            <Typography sx={{ pl: 2, fontSize: 14 }}>
              ...
              <a
                onClick={() => {
                  router.push('/assets/newPipelineUpload');
                }}>
                <b>here</b>
              </a>{' '}
              instead
      ...
      <div style={{ marginLeft: '3%', width: '90%', flexGrow: 2 }}>
                          ...
                          <MySelect
                            ...
                            inputProps={{
                              ...
                              onChange: (e: SelectChangeEvent<string>) =>
                                onSetModelType(index, e.target.value),
                        ...
                        <FormControl sx={{ m: 1, minWidth: 300 }} size="small">
                          <MySelect
                            ...
                            inputProps={{
                              multiline: true,
                              onChange: (e: SelectChangeEvent<string>) =>
                                onSetFolderModelType(index, e.target.value),
                              placeholder: 'Model Type',
                              defaultValue: 'Classification',
                            }}
                            items={['Classification', 'Regression']}
        ...
        <Container
          ...
          <Button
            ...
            data-testid="upload-models-button">
...
function ModelsValidating({
  filesValidating,
  updateFile,
  onBackClick,
}: ModelsValidatingProps) {
  const router = useRouter();
  const [focus, setFocus] = useState<(ModelFile & { _id?: string }) | null>(
    null
  );
  const [edit, setEdit] = useState<(ModelFile & { _id?: string }) | null>(null);
  const [openDialog, setOpenDialog] = useState<boolean>(false);
  const [modelsValidating, setModelsValidating] = useState<
    (ModelFile & { _id?: string })[]
  >([]);
  ...
  useEffect(() => {
    const stillValidating = modelsValidating.find(
      (e: ModelFile) => e.status.toString() === FileStatus.PENDING
    );
    if (stillValidating) {
        ...
        <div
          ...
          <Box
            ...
            <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
              {!isFailed &&
                modelsValidating &&
                modelsValidating.map((modelValidating) => (
                  <div
                    key={modelValidating._id}
                    onClick={() => onSetFocus(modelValidating)}
                    style={{
                      backgroundColor:
                        modelValidating._id == focus?._id ? '#F3F0F5' : '',
                    }}>
                    <ListItemButton>
                      <ListItemAvatar>
                      ...
                      <ListItemText
                        primary={modelValidating.name}
                        secondary={modelValidating.size}
                      />
                      <ListItemIcon>
                        <IconButton
                          sx={{
      ...
      {edit && (
        <Dialog
          ...
          <DialogActions>
            ...
            <Button onClick={() => onUpdateModel(edit._id, edit)}>
              Save & Close
            </Button>
          </DialogActions>
        </Dialog>
...
export default function NewModelUploadModule({
  ...
  useSubscription(VALIDATE_MODEL_UPDATED, {
    ...
    onData: (payload) => {
      ...
      if (
        data == undefined ||
        data.data.validateModelStatusUpdated == undefined
      )
        return;
  ...
  const updateFile = (
    modelFile: Partial<ModelFile> & { _id?: string }
  ): void => {
    setFilesValidating((current) => {
      // going into this code block, updatedFileInfo.status is either Valid or Invalid
      const updatedFilesValidating = current.map((file) => {
      ...
      <Paper
        ...
        {isValidating == false && (
          <ModelsPicking
            ...
            updateFileList={updateFileList}></ModelsPicking>
        )}
        {isValidating == true && (
          <ModelsValidating
            filesValidating={filesValidating}
            updateFile={updateFile}
            onBackClick={handleBackIconClick}></ModelsValidating>
        )}
      </Paper>
    );
  }
          ...
          <Container maxWidth="xl" className="mainContainer">
            <Paper
              ...
              {isValidating == true && (
                <ModelsValidating
                  filesValidating={filesValidating}
                  updateFile={updateFile}></ModelsValidating>
              )}
            </Paper>
          </Container>
        </Container>
