...
function StatusBadge({ reportStatus, projectId }: Partial<ProjectStatusProps>) {
  ...
  const onStatusClick = () =>
    projectId != undefined ? router.push(`/reportStatus/${projectId}`) : null;
...
function ProjectStatus({
  ...
  const [isRunningTestsOrGenerating, setIsRunningTestsOrGenerating] =
    useState(false);
  ...
  useEffect(() => {
    ...
    setIsRunningTestsOrGenerating(
      reportStatus === ProjectReportStatus.GeneratingReport ||
        reportStatus === ProjectReportStatus.RunningTests
    );
  }, []);
...
export default function ProjectBox({
  ...
  const [updatedReport, updatedReportLoading] = useSubscribeReportStatusUpdated(
    project.id as string
  );
  const [showDeleteConfirmation, setShowDeleteConfirmation] =
    useState<boolean>(false);
  const [lastModifiedDateDisplay, setLastModifiedDateDisplay] =
    useState<string>('');
  ...
  useEffect(() => {
    if (updatedReportLoading) return;
    ...
    <Grid xs={4}>
      <div
        ...
        <div
          ...
          <Tooltip
            title={
              <Typography fontSize={18}>{project.projectInfo.name}</Typography>
            }
          ...
          <Tooltip title="View Report" placement="right-start">
            <div>
              <IconButton
                ...
                disabled={
                  report?.status !== ProjectReportStatus.ReportGenerated
                }
      ...
      {showDeleteConfirmation ? (
        <ConfirmationDialog
          ...
          <p style={{ height: 100, textAlign: 'center', marginTop: '20px' }}>
            Permanently remove the following project
            <br />{' '}
            <Typography
              variant="heading5"
              color="text.strong"
