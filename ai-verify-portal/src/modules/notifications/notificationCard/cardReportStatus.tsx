...
function CardReportStatus(
  props: SpecializedCardProps<ReportStatusNotification>
) {
  const { notification, showMsgBody = false } = props;
  const { subject, body, projectId, projectName, reportStatus } = notification;
    ...
    case ProjectReportStatus.ReportGenerated:
      ...
      SubjectDisplay = function Comp() {
        return !projectName ? (
          ...
          <div>
            Report from <span style={{ fontWeight: 600 }}>{projectName}</span>{' '}
            is ready
          </div>
        );
      };
      CallToAction = function Comp() {
        ...
        return (
          <div
            className={styles.viewReportCTA}
