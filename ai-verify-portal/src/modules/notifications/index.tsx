...
import { StandardNotification, NotificationType, ReadStatus, ReportStatusNotification, AssetType, AssetStatusNotification } from './types';
import { useSubscription } from '@apollo/client';
import { ProjectReportStatus, Report } from 'src/types/project.interface';
import { REPORTSTATUS_SUBSCRIPTION, TESTTASK_SUBSCRIPTION, VALIDATEDATASET_SUBSCRIPTION, VALIDATEMODEL_SUBSCRIPTION } from './providers/gql';
import { TestEngineTask, TestEngineTaskStatus } from 'src/types/test.interface';
import { soundClick } from './sounds/soundClick';
import { AssetValidationStatus, DatasetStatusUpdate, ModelStatusUpdate } from 'src/types/dataset.interface';
...
function Notifications(props: NotificationsProps) {
  ...
  const [ selectedMessage, setSelectedMessage ] = useState<StandardNotification | undefined>();
  ...
  const [ flyoutMessage, setFlyoutMessage ] = useState<StandardNotification | undefined>();
  const notifRef = useRef<HTMLDivElement>(null);
  const flyOutTimer = useRef<NodeJS.Timeout>(); 
  const isDelete = useRef(false);
  const latestTimestamp = useRef<number | undefined>(undefined);
  const initialLoadDone = useRef(false);
  const unread = notificationsData !== undefined ? notificationsData.filter(notif => notif.readStatus === ReadStatus.NEW) : [];
  const sortedNotifications = notificationsData !== undefined ? [...notificationsData] : [];
  sortedNotifications.sort((a, b) => a.timestamp > b.timestamp ? -1 : 1);
  const { data: reportSubData, error: reportSubError } = useSubscription<{ reportStatusUpdatedNoFilter: Report }>(REPORTSTATUS_SUBSCRIPTION);
  const { data: testSubData, error: testSubError } = useSubscription<{ testTaskUpdatedNoFilter: TestEngineTask}>(TESTTASK_SUBSCRIPTION);
  const { data: datasetSubData, error: datasetSubError } = useSubscription<{ validateDatasetStatusUpdated: DatasetStatusUpdate}>(VALIDATEDATASET_SUBSCRIPTION);
  const { data: modelSubData, error: modelSubError } = useSubscription<{ validateModelStatusUpdated: ModelStatusUpdate}>(VALIDATEMODEL_SUBSCRIPTION);
  ...
  function handleCardClick(notification: StandardNotification | ReportStatusNotification | AssetStatusNotification) {
    setTimeout(() => { // requeue so that bindDocClickHandler executes first
    ...
    if (reportSubData && reportSubData.reportStatusUpdatedNoFilter) {
      ...
      if (report.status === ProjectReportStatus.ReportGenerated) {
        try {
          ...
          console.error(`notificationsContext:createNotificaiton on report event - ${err}`);
        }
      }
    }
  }, [reportSubData]);
    ...
    if (testSubData && testSubData.testTaskUpdatedNoFilter) {
      ...
      if (testTask.status === TestEngineTaskStatus.Error) {
        try {
          ...
          const errors = testTask.errorMessages ? testTask.errorMessages.map(err => err.description) : [];
          ...
          console.error(`notificationsContext:createNotificaiton on testtask event - ${err}`);
        }
      }
    }
  }, [testSubData]);
  ...
  useEffect(() => {
    if (datasetSubData && datasetSubData.validateDatasetStatusUpdated) {
      ...
      if (dataset.status === AssetValidationStatus.Error || dataset.status == AssetValidationStatus.Invalid) {
        try {
          ...
          console.error(`notificationsContext:createNotificaiton on dataset validation event - ${err}`);
        }
        return;
      }
  ...
  useEffect(() => {
    if (modelSubData && modelSubData.validateModelStatusUpdated) {
      ...
      if (model.status === AssetValidationStatus.Error || model.status === AssetValidationStatus.Invalid) {
        try {
          ...
          console.error(`notificationsContext:createNotificaiton on model validation event - ${err}`);
        }
        return;
      }
  ...
  useEffect(() => {
    ...
    if (reportSubError)
      console.error(`"report generation" Subscription error - ${reportSubError}`);
    if (datasetSubError)
      console.error(`"validate dataset" subscription error - ${datasetSubError}`);
    if (datasetSubError)
  ...
  useEffect(() => {
    ...
    if (sortedNotifications.length && sortedNotifications[0].readStatus == ReadStatus.DONE) return;
    if (initialLoadDone.current === false ) {
      initialLoadDone.current = true;
      if (latestTimestamp.current == undefined && sortedNotifications[0]) {
        latestTimestamp.current = new Date(sortedNotifications[0].timestamp).getTime();
      }
      return;
    }
    // --
    ...
    if (latestTimestamp.current == undefined) {
      ...
      if (sortedNotifications[0] && new Date(sortedNotifications[0].timestamp).getTime() > latestTimestamp.current) {
  ...
  useEffect(() => {
    ...
    setShowPanel(false);
    if (flyoutMessage.type === NotificationType.REPORT_STATUS)
      soundBell();
    else
      soundClick();
    if (flyOutTimer.current) clearTimeout(flyOutTimer.current);
  ...
  return <div className={styles.root} style={style} ref={notifRef}>
    <div className={styles.icon} onClick={handleMainIconClick}>
      <IconButton
        noOutline
        iconFontSize={25}
        iconComponent={NotificationsNoneIcon}/>
      {unread.length ? <div className={styles.countBadge}>{unread.length}</div> : null}
    </div>
    { showPanel ? <div className={styles.panel}>
        <div className={styles.header}>
          <h3>Notifications</h3>
          <div className={styles.countBadgeAll}>{sortedNotifications.length}</div>
        </div>
        <div className={styles.list}>
          { selectedMessage ? <div className={styles.msgNav}>
            <div className={styles.msgNavBtn} onClick={handleMsgBackClick}>
              <Icon name={IconName.CHEVRON_LEFT} size={12}/>
              <div style={{ textAlign: 'start', marginLeft: '5px' }}>back</div>
            </div>
          </div> : null }
          <div className={styles.scrollContainer}>
            { notificationsData && !notificationsData.length ? <div className={styles.emptyList}>
              No notifications
            </div> : null }
            { selectedMessage ? <NotificationCard
                hideDeleteIcon
                notification={selectedMessage}
                showMsgBody
                onDeleteClick={handleDeleteClick}
              /> : null }
            { !selectedMessage ? sortedNotifications.map(msg => <NotificationCard 
                notification={msg}
                key={msg.id}
      ...
      <div className={clsx(styles.flyOutMsg, flyoutMessage ? styles.flyIn : null)}>
      { flyoutMessage ? <NotificationCard 
          hideDeleteIcon
          notification={flyoutMessage}
          onClick={handleFlyoutClick}
          onDeleteClick={handleFlyoutClick}/> : null}
      </div>
  </div>
}
