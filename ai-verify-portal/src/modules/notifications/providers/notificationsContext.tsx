...
const NotificationsStateContext = createContext<
    { 
      notificationsData: (StandardNotification | ReportStatusNotification)[] | undefined,
...
function NotificationsProvider({children}: NotificationsProviderProps) {
  const [notificationsData, setNotificationsData] = useState<(
    StandardNotification | ReportStatusNotification | AssetStatusNotification
  )[]>();
  const [addNotificationToDB] = useMutation<GqlCreateNotificationResult>(GQL_CREATE_NOTIFICATION);
  const [updateNotificationReadStatusInDB] = useMutation<GqlUpdateReadStatusResult>(GQL_UPDATE_NOTIFICATION_READ);
  const [deleteNotificationFromDB] = useMutation<GqlDeleteResult>(GQL_DELETE_NOTIFICATION);

  async function createNotification(newNotif: CreateNewNotificationInput) {
    let notif: Omit<StandardNotification, 'id'> | Omit<ReportStatusNotification, 'id'> | Omit<AssetStatusNotification, 'id'>;
    ...
    if (newNotif.type === NotificationType.REPORT_STATUS) {
      ...
      if ('reportStatus' in newNotif) {
        ...
        console.error('Cannot create Report Status Notification. "reportStatus" is required');
        return;
      }
      ...
      if ('projectId' in newNotif) {
        ...
        console.error('Cannot create Report Status Notification. "projectId" is required');
        return;
      }
      ...
      if ('projectName' in newNotif) {
        ...
        console.error('Cannot create Report Status Notification. "projectName" is required');
        return;
      }
    ...
    } else if (newNotif.type === NotificationType.DATASET_STATUS || newNotif.type === NotificationType.MODEL_STATUS) {
      let assetStatus: AssetValidationStatus | undefined ;
      let assetName: string | undefined ;

      const assetType = newNotif.type === NotificationType.DATASET_STATUS ? AssetType.DATASET : AssetType.MODEL;

      if ('assetStatus' in newNotif) {
        assetStatus = newNotif.assetStatus;
      } else {
        console.error('Cannot create Asset Status Notification. "assetStatus" is required');
        return;
      }
      ...
      if ('assetName' in newNotif) {
        ...
        console.error('Cannot create Asset Status Notification. "assetName" is required');
        return;
      }
      ...
      if (newNotif.projectId !== undefined) {
        console.error(`"projectId" is ignored in Standard Notification. Did you mean to create notification type "${NotificationType.REPORT_STATUS}"`);
      }
      if (newNotif.projectName !== undefined) {
        console.error(`"projectName" is ignored in Standard Notification. Did you mean to create notification type "${NotificationType.REPORT_STATUS}"`);
      }
      if (newNotif.reportStatus !== undefined) {
        console.error(`"reportStatus" is ignored in Standard Notification. Did you mean to create notification type "${NotificationType.REPORT_STATUS}"`);
      }
  ...
  async function updateNotificationStatusToRead(id: string) {
    ...
    try {
      result = await updateNotificationReadStatusInDB({ variables: { id, readStatus: ReadStatus.DONE }});
      if (!result.data) {
        console.log(`There was a problem in trying to update "readStatus" - ${result}`);
      }
    } catch(err) {
      console.error(err);
    }
    ...
    setNotificationsData(
      produce((draft) => {
        ...
        if (!result || !result.data || !result.data.updateNotificationReadStatus.id) return;
  ...
  async function deleteNotification(id: string) {
    ...
    try {
      ...
      if (!result.data) {
        console.log(`There was a problem in trying to delete notification - ${result}`);
      }
    } catch(err) {
      console.error(err);
    }
  ...
  return <NotificationsStateContext.Provider value={contextValue}>
    {children}
