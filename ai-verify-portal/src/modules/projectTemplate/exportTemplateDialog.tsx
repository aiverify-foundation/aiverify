...
export default function ExportTemplateDialog(props: ExportTemplateDialogProps) {
  ...
  const [templateCID, setTemplateCID] = useState(
    projectStore.projectInfo.name.toLowerCase().replaceAll(/[^a-z0-9]/g, '-')
  );
  ...
  async function handleExport() {
    if (pluginGID.trim().length == 0 || templateCID.trim().length == 0)
      ...
      return;
    try {
      const response = await projectStore.exportTemplate(
        pluginGID.trim(),
        templateCID.trim()
      );
      if (response === 'ok') {
        ...
        {showError ? (
          <div className={styles.resultBody}>
            ...
            <div className={styles.resultContent}>
              ...
              <div>
                There was a problem exporting this design as a template plugin.
              </div>
            </div>
          </div>
        ) : null}
        {!showSuccess && !showError ? (
          <div>
            <MyTextField
              ...
              errorText={
                pluginGID.length === 0 ? 'Plugin GID is required' : null
              }
              FormControlProps={{
                sx: { mb: 2 },
              }}
            ...
            <MyTextField
              ...
              errorText={
                templateCID.length === 0 ? 'Template GID is required' : null
              }
              FormControlProps={{
                sx: { mb: 2 },
              }}
