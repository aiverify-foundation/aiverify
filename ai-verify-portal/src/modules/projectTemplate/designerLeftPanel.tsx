...
import { Accordion, AccordionDetails, InputAdornment, Link, TextField } from "@mui/material";
...
import AIFPlugin, { ReportWidget, ReportWidgetStatus } from "src/types/plugin.interface";
...
type WidgetGroupProps = {
  widget: ReportWidget,
  disabled?: boolean,
  onDragStart: (widget: ReportWidget) => React.DragEventHandler<HTMLDivElement> | undefined;
}
  ...
  function WidgetDetails(props: WidgetDetailsProps, scrollContainerRef) {
    ...
    return <div
      ...
      className={styles.widgetDetailsScrollContainer}>
          <div className={styles.widgetDetailsRow} style={{ flexDirection: 'column'}}>
        ...
        {hasTags ? 
          <div className={styles.widgetDetailsRow}>
            ...
            <div className={styles.widgetDetailText}>{widget.tags.join(', ')}</div>
          </div> : null} 
        {hasDependencies ? (
          <div className={styles.widgetDetailsRow} style={{ flexDirection: 'column'}}>
...
function DraggableWidget(props: PropsWithChildren<WidgetGroupProps>) {
  ...
  return <div
    key={widget.gid}
    ref={divRef}
    className={clsx(styles.draggableWidget, disabled ? styles.draggableWidget_disabled : null)}
    draggable={!disabled}
    unselectable="on"
  ...
  > 
    <div className={styles.widgetHeadingWrapper}>
      ...
      {!disabled ?
          <div className={styles.draggableHandleIconWrapper} style={{ display: 'flex' }}>
...
function DesignerLeftPanel(props: LeftPanelProps) {
  ...
  function handleSearchInputChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (e.target.value.trim() === '') {
      setPluginsWithWidgets(projectStore.pluginManager.plugins.filter(
  ...
  useEffect(() => {
    widgetsFuseRef.current = new Fuse(projectStore.pluginManager.plugins.filter(
      plugin => plugin.reportWidgets && plugin.reportWidgets.length > 0), fuseSearchOptions);
  }, []);
      ...
      <div className={styles.stickyLeftPanel}>
        <div style={{ display: 'flex', flexDirection: 'column'}}>
          <div className={styles.panelSection}>
            <div>
              ...
              <div 
                ...
                style={{ lineHeight: 1.2 }}>{projectStore.projectInfo.name}
              </div>
            </div>
          </div>
          ...
          <div className={styles.panelSectionHeading}>
            <WidgetsIcon fontSize='small' sx={{ color: '#c42983', marginTop: '4px', marginRight: '10px'}}/>
                ...
                <Accordion disableGutters>
                  ...
                  <AccordionDetails>
                    {plugin.reportWidgets && plugin.reportWidgets.map(widget => {
                      const isWidgetDisabled = projectStore.isReadonly || widget.status !== ReportWidgetStatus.OK;
                      return (
                        <DraggableWidget
                          key={widget.gid}
                          widget={widget}
