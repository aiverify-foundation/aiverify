...
export default function TemplateDetail(props: TemplateDetailProps) {
  ...
  const dateTimeDisplay = template.updatedAt
  ...
  return (
    <div
      ...
      <div className={styles.mainContent} onClick={onClick}>
        <Tooltip
          content={template.projectInfo.name}
        ...
        {template.projectInfo.description ? (
          <Tooltip
            content={template.projectInfo.description}
        ...
        {dateTimeDisplay ? (
          <div className={styles.updatedAt}>Updated on {dateTimeDisplay}</div>
        ) : null}
      </div>
      ...
      <div className={styles.btnColumn}>
        {template.fromPlugin ? (
          <Tooltip
            ...
            <IconButton
              ...
              onClick={() =>
                router.push(`/projectTemplate/${template.id}`)
              }></IconButton>
          </Tooltip>
        ) : (
          <Tooltip
            content="Edit Project Template"
            backgroundColor="#676767"
            fontColor="#FFFFFF"
            offsetLeft={-20}>
            <IconButton
        ...
        <Tooltip
          content="Copy Project Template to new instance"
          backgroundColor="#676767"
          fontColor="#FFFFFF"
          offsetLeft={-20}>
          <IconButton
            ...
            iconFontSize={iconSize}
            style={{ margin: '10px 0px' }}
            onClick={onCloneBtnClick}></IconButton>
        </Tooltip>
        <Tooltip
          content="Delete Project Template"
          backgroundColor="#676767"
          fontColor="#FFFFFF"
          offsetLeft={-20}>
          <IconButton
            ...
            disabled={template.fromPlugin}></IconButton>
        </Tooltip>
      </div>
    </div>
  );
