...
export default function ReportWidgetComponent({ mdxBundle, inputBlockData, result, meta, properties, report }: ReportWidgetComponentProps) {
	const ref = useRef<HTMLDivElement>(null);
	const { code, frontmatter } = mdxBundle
	const Component = useMemo(() => getMDXComponent(code), [code])
	const tests = report.tests || [];
	const timeTaken = report.timeTaken>report.totalTestTimeTaken?report.timeTaken:report.totalTestTimeTaken;
	...
	useEffect(() => {
		...
		const obj = Object.freeze({
			...
			getResults(cid: string, gid:(null|string)=null) {
				const key = (gid && gid.length > 0)?`${gid}:${cid}`:`${meta.pluginGID}:${cid}`;
				return result[key];
			},
