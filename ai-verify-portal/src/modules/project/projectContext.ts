...
import { useCreateProject, useCreateProjectFromTemplate, useUpdateProject, useGenerateReport, useCancelTestRuns } from 'src/lib/projectService';
...
export function useProjectStore(data: Project, pluginManager: PluginManagerType): ProjectStore {
	...
	// Input block data
	// @ts-ignore
	const [inputBlockData, _dispatchInputBlockData] = useReducer(templateStore.mapReducer<any>, data.inputBlockData || {});
	...
	// Test Information
	// @ts-ignore
	const [testInformationData, _dispatchTestInformationData] = useReducer(templateStore.mapReducer<TestInformation>, {}, () => {
		if (!data.testInformationData)
			return {};
		return data.testInformationData.reduce<GenericMap<TestInformation>>((acc, info) => {
	...
	// @ts-ignore
	const [modelAndDatasets, _dispatchModelAndDatasets] = useReducer(templateStore.updateReducer<any>, data.modelAndDatasets || {});
	...
	const isAlgorithmValid = (gid: string): boolean => {
		const algo = templateStore.dependencies.algorithms.find(algo => algo.gid == gid);
		if (!algo) {
	...
	const [reportStatus, setReportStatus] = useState<ProjectReportStatus>(data.report?data.report.status:ProjectReportStatus.NotGenerated)
	...
	const createProject = () => {
		return new Promise<string>((resolve, reject) => {
			...
			createProjectFn(project).then((result: string) => {
