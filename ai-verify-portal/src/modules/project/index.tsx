...
import { DatasetModelFilePicker, FileSelectMode } from './datasetModelFilePicker';
import { ReportDesignerHeader } from './header';
import { formatDate } from 'src/lib/utils';
import { AlertBox, AlertBoxFixedPositions, AlertBoxSize } from 'src/components/alertBox';
...
export default function ProjectModule({ data, pluginManager }: Props) {
  ...
  const [step, setStep] = useState(projectStore.isNew ? ProjectStep.CaptureProjectInfo : ProjectStep.SelectTemplate);
  const [errorMessage, setErrorMessage] = useState("");
  const [showSelectAlgoDialog, setShowSelectAlgoDialog] = useState(false);
  const [ selectedProjectTemplateId, setSelectedProjectTemplateId ] = useState<string | 'blank' | undefined>();
  ...
  async function goForward() {
    switch(step) {
      case ProjectStep.CaptureProjectInfo:
        setStep(projectStore.isNew ? ProjectStep.SelectTemplate : ProjectStep.DesignReport);
        break;
      case ProjectStep.SelectTemplate:
        ...
        if (projectStore.isNew) {
          const id = await projectStore.createProjectFromTemplate(selectedProjectTemplateId);
          if (id)
            router.push(`/project/${id}`);
        }
        break;
      case ProjectStep.CaptureTestInput:
        if (projectStore.reportStatus === ProjectReportStatus.GeneratingReport) {
          router.push(`/reportStatus/${data.id}`)
        } else {
          setValidAlgos(projectStore.dependencies.algorithms.filter(algo => projectStore.isAlgorithmValid(algo.gid)));
          setInvalidAlgos(projectStore.dependencies.algorithms.filter(algo => !projectStore.isAlgorithmValid(algo.gid)));
  ...
  const generateReport = () => {
    ...
    projectStore.generateReport(gids).then(() => {
  ...
  useEffect(() => {
    ...
    if (step === ProjectStep.SelectTemplate && selectedProjectTemplateId == undefined) {
      setDisableNextBtn(true);
    }
  }, [step]);
    ...
    <div>
      ...
      <div className="layoutContentArea">
        <div className="scrollContainer">
          {step === ProjectStep.CaptureProjectInfo && <ProjectInformationComponent
            projectStore={projectStore}
            onProjectInfoChange={handleProjectInfoChange}
            />}
          {step === ProjectStep.SelectTemplate && <SelectTemplate
            onProjectTemplateSelected={handleProjectTemplateSelected}
            selectedId={selectedProjectTemplateId}/>}
          {step === ProjectStep.DesignReport && <CanvasComponent projectStore={projectStore} />}
          {step === ProjectStep.CaptureTestInput && <UserInputComponent projectStore={projectStore}
        ...
        {projectStore.dependencies.algorithms && showSelectAlgoDialog ?
          ...
          <AlertBox.Body hasFooter bodyStyles={{ minHeight: 100 }}>
            {invalidAlgos.length > 0 ? <div>
                The following algorithms has invalid arguments and cannot be run:
                <List dense={true} sx={{ listStyleType: 'disc', pl: 2, pt:0 }}>
                  {invalidAlgos.map(algo => (
                    <ListItem key={`invalid-algo-${algo.gid}`} disablePadding sx={{ display: 'list-item' }}>{algo.name}</ListItem>
                  ))}
                </List>
              </div> : null}
            {validAlgos.length > 0 ? <div>
                The following algorithms will be run:
                <List dense={true} sx={{ listStyleType: 'disc', pl: 2, pt:0 }}>
                  {validAlgos.map(algo => (
                    <ListItem key={`valid-algo-${algo.gid}`} disablePadding sx={{ display: 'list-item' }}>{algo.name}</ListItem>
                  ))}
                </List>
              </div> : <p>No algorithms to run. Click Proceed to generate report.</p>}
          </AlertBox.Body>
          <AlertBox.Footer>
            <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end'}}>
              <div>
                <button className="aivBase-button aivBase-button--secondary aivBase-button--small" onClick={() => setShowSelectAlgoDialog(false)}>Cancel</button>
                <button className="aivBase-button aivBase-button--primary aivBase-button--small" onClick={() => generateReport()}>Proceed</button>
              </div>
            </div>
          </AlertBox.Footer>
        </AlertBox> : null}
