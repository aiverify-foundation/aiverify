...
import { useCloneProjectTemplate, useDeleteProjectTemplate, useGetProjectTemplates } from 'src/lib/projectTemplateService';
...
function BlankCanvasCard(props: BlankCanvasCardProps) {
  ...
  return (
    <div className={clsx (styles.blankCanvasCard, highlighted ? styles.card__highlighted : null)}>
			<div className={styles.blankCardmainContent} onClick={onClick}>
				<div className={styles.blankCardHeading}>Blank Canvas</div>
				<div className={styles.blankCardDescription}>Design your own report by dragging widgets onto a blank canvas</div>
			</div>
...
function SortMenu(props: SortMenuProps) {
  ...
  return <div ref={menuRef} className={sortMenuStyles.sortMenu}>
    <div className={sortMenuStyles.label} style={{  }}>Sort by</div>
    <div className={sortMenuStyles.optionDisplay} onClick={handleSortMenuClick}>{sortDisplayText}
      <div className={sortMenuStyles.icon}>{showMenu ? <ArrowDropUpIcon /> : <ArrowDropDownIcon />}</div>
    </div>
    { showMenu ? 
      <ListMenu containerStyles={{ position: 'absolute', zIndex: 101, fontSize: '13px', right: '0px', top: '36px', width: '220px' }}>
        <ListMenuItem id="dateAsc" displayText='Template Name (asc)' onClick={handleMenuItemClick(SortOption.TemplateNameAsc)}/>
        <ListMenuItem id="dateDesc" displayText='Template Name (desc)' onClick={handleMenuItemClick(SortOption.TemplateNameDesc)}/>
      </ListMenu> : null
    }
  </div>
}
...
export default function SelectTemplate(props: TemplatesListProps) {
  ...
  const [ selectedSortOption, setSelectedSortOption ] = useState<SortOption>(SortOption.TemplateNameAsc);
  ...
  useEffect(() => {
    if (!templates || !initialTemplatesLoaded) return;
      ...
      switch (selectedSortOption) {
        case SortOption.TemplateNameDesc:
          sortedTemplates.sort((a, b) => (a.projectInfo.name > b.projectInfo.name ? -1 : 1))      
          break;
        case SortOption.TemplateNameAsc:
          sortedTemplates.sort((a, b) => (a.projectInfo.name < b.projectInfo.name ? -1 : 1))      
          break;
  ...
  useEffect(() => {
    if (!data) return;
    templatesFuseRef.current = new Fuse(data.projectTemplates, fuseSearchOptions);
    setTemplates(data.projectTemplates);
      ...
      <div className={styles.layout}>
        <div style={{ marginBottom: '25px'}}>
          <div style={{ display: 'flex' }}>
            <MenuBookIcon style={{ fontSize: '30px', marginRight: '15px', marginTop: '2px' }} />
            <div>
              <h3 className='screenHeading'>Select Report Template</h3>
              <p className='headingDescription'>Select a template or start with an empty canvas.</p>
            </div>
          </div>
          {!templates.length ? 
          <div style={{ color: '#c42983', display: 'flex', marginTop: '15px', fontSize: '14px' }}> 
        ...
        {!loading && error ? <PageLevelErrorAlert
          ...
          {data && data.projectTemplates.length ? <div className={styles.searchRow}>
            <div className={styles.searchInput}>
              ...
              {templates.length ? <SortMenu selected={selectedSortOption} onClick={handleSortSelected}/> : null}
          </div>: null}
          ...
          {templates ? templates.map(template => {
            ...
            return <TemplateDetail
              ...
              onDeleteBtnClick={handleDeleteTemplateClick(template.id)}
