...
function calculateCSSWidth(muiWidth: InputBoxWidths | undefined): string | undefined {
  if (!muiWidth) return;
  switch (muiWidth) {
...
export default function InputBlockList (props: InputBlockListProps) {
  const { projectStore, userInputList, inputBlockStates, setInputBlockStates } = props;
  const [ currentState, setCurrentState ] = useState<InputBlockState|null>(null);
  ...
  const showInputBlock = (gid: string): void => {
    ...
    if (currentState && (currentState.inputBlock.gid === gid)) {
      ...
      setInputBlockContext({
        data,
        meta: {},
        onChangeData: (key: string, value: any) => _onChangeData(gid, key, value),
      })
  ...
  const closeInputBlock = (): void => {
    if (!currentState) // should not happen
      return;
    const state = currentState;
    const gid = state.inputBlock.gid;
    state.open = false;
  ...
  const _onChangeData = (gid: string, key: string, value: any) => {
    const data = projectStore.inputBlockData[gid] || {};
    data[key] = value;
    projectStore.dispatchInputBlockData({ type:MapActionTypes.SET, key:gid, payload:data })
  }
  ...
  return (
    <div className={styles.inputSection}>
      <div className={styles.sectionHeading}>
        ...
        <p className='headingDescription'>Open each input block to provide the information required</p>
      </div>
      <Box>
        {userInputList.map(input => {
          if (typeof input === 'string') {
            return <div className={styles.groupTitle} key={`summarygroup-${input}`}>{input}</div>
          } else if (Array.isArray(input)) {
            const iblocks = input as InputBlock[];
            return (
              <div className={styles.iBlockGroup} key={`group-${input[0].group}`}>
                <div className={styles.iBgroupLine} />
                {iblocks.map(ib => <IblockCard
                  key={`summary-${ib.gid}`}
                  iBlock={ib}
        ...
        <AlertBox
          ...
          containerStyles={{
            width: currentState.inputBlock.fullScreen ? 
              '98vw' : calculateCSSWidth(currentState.inputBlock.width),
