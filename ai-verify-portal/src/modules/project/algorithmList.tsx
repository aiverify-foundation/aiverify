...
export default function AlgorithmList (props: AlgorithmListProps) {
  ...
  const [ currentState, setCurrentState ] = useState<TestInformation | null>(null);
  ...
  function handleDataChange(gid: string) {
    return (data: object) => {
      ...
      projectStore.dispatchTestInformationData({ type:MapActionTypes.SET, key:gid, payload })
  ...
  return (
    <div className={styles.inputSection}>
      <div className={styles.sectionHeading}>
        ...
        <p className='headingDescription'>Open each test argument to provide the arguments required to run the tests and generate the report</p>
      </div>
        {algorithms.map(algo => {
          const hasInputProperties = algo.inputSchema.properties && Object.keys(algo.inputSchema.properties).length;
          const isAlgoValid = projectStore.isAlgorithmValid(algo.gid);
          return <div id={`algocard-${algo.gid}`} className={styles.inputCard} key={`algo-${algo.gid}`}>
            ...
            <div className={styles.inputMain}>
              ...
              {hasInputProperties ? <div className={styles.buttonRow}>
                <button
                  className='aivBase-button aivBase-button--primary aivBase-button--small'
                  onClick={handleOpenBtnClick(algo)}>Open</button>
                {!isAlgoValid ? <div className={styles.errorMsg}>Invalid Arguments</div> : null}
              </div> : <div>This test does not require any arguments input.</div>}
            </div>
          </div>
        })}
