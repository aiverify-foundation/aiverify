...
function DatasetModelFilePicker(props: DatasetModelFileSelectorProps) {
  ...
  function fileSelectedHandler(selectedFile: Dataset & { __typename?: string} | Model & { __typename?: string} ) {
    ...
    if (mode === FileSelectMode.DATASET) {
      ...
      if (payload.testDataset.dataColumns) {
        payload.testDataset.dataColumns = payload.testDataset.dataColumns.map(datCol  => {
          const { __typename, ...rest } = datCol as DatasetColumn & { __typename?: string };
          return rest;
        })
      }
      ...
      if (payload.groundTruthDataset.dataColumns) {
        payload.groundTruthDataset.dataColumns = payload.groundTruthDataset.dataColumns.map(datCol  => {
          const { __typename, ...rest } = datCol as DatasetColumn & { __typename?: string };
          return rest;
        })
      }
  ...
  if (mode === FileSelectMode.DATASET || mode === FileSelectMode.GROUNDTRUTH) {
    ...
    return showNewDataset ? <NewDatasetModule withoutLayoutContainer onBackIconClick={handleNewDatasetBackIconClick}/> : <div className={styles.container__limits}>
      <div className={styles.inputsLayout} style={{ height: '600px', paddingTop: '24px' }}>
      <div style={{ 
        marginBottom: '15px',
        display: 'flex',
        justifyContent: 'space-between'
          ...
          <h3 className='screenHeading'>
            {mode === FileSelectMode.DATASET ? 'Choose the Dataset' : 'Choose Ground Truth Dataset'}
          </h3>
          <button
      ...
      <DatasetListComponent containerStyles={{
        padding: 0,
        height: '100%',
        backgroundColor: '#FFF',
        margin: 0,
        ...
        showSelectDatasetBtn onDatasetSelected={fileSelectedHandler} />
        </div>
    </div>
  }
  ...
  if (mode === FileSelectMode.MODEL) {
    if (showNewModel) {
      return <NewModelModule withoutLayoutContainer onBackIconClick={handleNewModelBackIconClick}/>
    } else {
      return <div className={styles.container__limits}>
              <div className={styles.inputsLayout} style={{ height: '600px', paddingTop: '24px' }}>
                ...
                <ModelListComponent containerStyles={{
                  padding: 0,
                  height: '100%',
                  backgroundColor: '#FFF',
                  margin: 0,
                  ...
                  showSelectModelBtn onModelSelected={fileSelectedHandler}/>
              </div>
              </div>
    }
  }
