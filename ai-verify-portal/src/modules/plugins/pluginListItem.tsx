...
function PluginListItem(props: PluginListItemProps) {
  ...
  return <div className={clsx('pluginList-card', styles.listItem, selected ? styles.listItem__selected : null)}
    ...
    <div className={styles.content}>
      <div style={{ minWidth: '240px' }}>
        ...
        <div className={styles.row}>
          <div className={styles.valueDisplay}>
            ...
            <div className={clsx(styles.value, 'listItem_installedDate')}>{dateTimeDisplay}</div>
          </div>
        </div>
        <div className={styles.row}>
          <div className={styles.valueDisplay}>
      ...
      <div>
        <div className={styles.componentCounts}>
          {noOfWidgets ? <div>widgets: <span className={styles.count}>{noOfWidgets}</span></div> : null}
          {noOfAlgos ? <div>algorithms: <span className={styles.count}>{noOfAlgos}</span></div> : null}
          {noOfIBlocks ? <div>input blocks: <span className={styles.count}>{noOfIBlocks}</span></div> : null}
          {noOfTemplates ? <div>templates: <span className={styles.count}>{noOfTemplates}</span></div> : null}
        </div>
      </div>
    </div>
  </div>
