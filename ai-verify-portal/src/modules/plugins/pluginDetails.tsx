...
import { DependencyRequirement, DependencyStatusResult, getPythonPackageDependencyStatus } from './api/algorithms';
...
function PluginsDetails (props: PluginDetailsProps) {
  ...
  const [ algoRequirementsResult, setAlgoRequirementsResult ] = useState<DependencyStatusResult[]>();
  ...
  useEffect(() => {
    if (tabsGroupRef.current) {
      const firstTab = tabsGroupRef.current.querySelector<HTMLDivElement>('div:first-of-type');
      ...
      if (algorithms) {
        algorithms.forEach(algo => {
          if (algo.requirements.length) {
            algo.requirements.forEach(req => algoRequirementStrings.push(
              { requirement: extractRequirementString(req) }
            ))
          }});
          const result = await getPythonPackageDependencyStatus(algoRequirementStrings);
  ...
  useEffect(() => {
    ...
    if (panelRef.current) {
      const headEl = panelRef.current.querySelector<HTMLDivElement>(`#${headSectionID}`);
      if (headEl) {
    ...
    return algoRequirementsResult && algoRequirementsResult.length ? 
      <div className={styles.algoRequirements}>
        ...
        <div style={{ margin: '10px' }}>
          {algoRequirementsResult.map(depResult => <DependencyStatus
  ...
  return (
    <div data-testid="pluginDetailsPanel" className={styles.pluginDetailsPanel} ref={panelRef}>
      <div id={headSectionID} className={styles.detailsHead}>
        ...
        <div>
          {enableDeletebtn ?
            <button className="aivBase-button aivBase-button--outlined aivBase-button--small"
              onClick={onDeleteBtnClick}>Delete</button> : null}
        </div>
      </div>
      <div className={styles.detailsMain}>
        <div className={styles.tabs}>
          <div ref={tabsGroupRef} className={styles.tabsBtnGroup}>
            {templateCount > 0 ? <div className={clsx(
                styles.tabBtn,
                activeTab === PluginComponent.TEMPLATE ? styles.tabBtn__selected : null
              )}
              onClick={handleTabClick(PluginComponent.TEMPLATE)}>
              Templates<div className={styles.counter}>{templateCount}</div></div> : null }
            {widgetCount > 0 ? <div className={clsx(
                styles.tabBtn,
                activeTab === PluginComponent.WIDGET ? styles.tabBtn__selected : null)}
              onClick={handleTabClick(PluginComponent.WIDGET)}>
              Widgets<div className={styles.counter}>{widgetCount}</div></div> : null }
            {algoCount > 0 ? <div className={clsx(
                styles.tabBtn,
                activeTab === PluginComponent.ALGO ? styles.tabBtn__selected : null
              )}
              onClick={handleTabClick(PluginComponent.ALGO)}>
              Algorithms<div className={styles.counter}>{algoCount}</div></div> : null }
            {iBlockCount > 0 ? <div className={clsx(
                styles.tabBtn,
                activeTab === PluginComponent.IBLOCK ? styles.tabBtn__selected : null
              )}
              onClick={handleTabClick(PluginComponent.IBLOCK)}>
              Input Blocks<div className={styles.counter}>{iBlockCount}</div></div> : null }
          </div>
          <div className={styles.tabsDivider} />
          <div className={styles.tabContent}>
            <div ref={scollContainerRef} className={clsx(styles.scrollContainer)}>
              {activeTab === PluginComponent.TEMPLATE ?
                <DisplayTemplates plugin={plugin} /> : null}
              {activeTab === PluginComponent.WIDGET ?
                <DisplayReportWidgets plugin={plugin} /> : null}
              {activeTab === PluginComponent.ALGO ?
                <DisplayAlgorithms plugin={plugin}>
                  <AlgoDependencies />
                </DisplayAlgorithms> : null}
              {activeTab === PluginComponent.IBLOCK ?
                <DisplayInputBlocks plugin={plugin} /> : null}
          </div>
        </div>
        </div>
      </div>
