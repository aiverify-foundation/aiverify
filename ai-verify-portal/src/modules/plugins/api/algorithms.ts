...
async function handleResponseBody<T>(response: Response): Promise<ApiResult<T> | ErrorWithMessage> {
  if (response.ok) {
    ...
    return toErrorWithMessage({ status: response.status, data: response.statusText });
  }
}
...
async function getPythonPackageDependencyStatus(requirements: DependencyRequirement[]): 
