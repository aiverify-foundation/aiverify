...
async function fetchAllPlugins(): Promise<ApiResult<PluginManagerType> | ErrorWithMessage> {
  const response = await fetch(listURL);
  const result = await handleResponseBody<PluginManagerType>(response);
  return result;
}

async function uploadPlugin(file: File): Promise<ApiResult<AIFPlugin> | ErrorWithMessage> {
...
async function deletePlugin(pluginId: string): Promise<ApiResult<string> | ErrorWithMessage> {
  const options = {
    method: 'DELETE'
  }
