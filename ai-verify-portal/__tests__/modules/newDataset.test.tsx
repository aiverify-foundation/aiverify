...
describe('New Dataset', () => {
  ...
  describe('Initial Render', () => {
    ...
    it('should render the back button', () => {
      render(<MockProviders apolloMocks={mocks}><NewDatasetModule/></MockProviders>);
    ...
    it('should render the upload screen', () => {
      ...
      expect(screen.getByTestId("upload-file-dropbox")).toHaveTextContent("Drag & Drop or Click to Browse");
      expect(screen.getByTestId("upload-folder-button")).toHaveTextContent("Upload Folder");
      expect(screen.getByText('Selected Files')).toBeDefined();
      expect(screen.getByTestId("upload-datasets-button")).toHaveAttribute('aria-disabled', 'true');
    })
  ...
  describe('File Picking', () => {
    ...
    it('should enable the upload button when files are picked', async() => {
      const { container } = render(<MockProviders apolloMocks={mocks}><NewDatasetModule/></MockProviders>);
    ...
    it('should show error message when more than 10 files are selected', async() => {
      ...
      Object.defineProperty(input, 'files', {
        value: [file1, file2, file3, file4, file5, file6, file7, file8, file9, file10, file11]
      })
      act(() => {
        if (input){
          fireEvent.change(input)
      ...
      expect(screen.getByText('Maximum 10 files to be uploaded at once. Please select less files.')).toBeDefined();
    })
    ...
    it('should show error message if any file selected is more than 4GB', async() => {
      ...
      expect(screen.getByText('File selection error')).toBeDefined();
      expect(screen.getByText('Maximum file size is 4GB. Please upload smaller files.')).toBeDefined();
    })
