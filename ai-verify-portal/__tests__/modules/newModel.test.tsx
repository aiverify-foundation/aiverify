...
describe('New Model', () => {
  ...
  it('should render the back button', () => {
    render(<MockProviders><NewModelModule/></MockProviders>);
  ...
  it('should render the upload/ api option buttons', () => {
    render(<MockProviders><NewModelModule/></MockProviders>);
    expect(screen.getByTestId("new-model-option")).toHaveTextContent("Upload AI Model");
    expect(screen.getByTestId("new-pipeline-option")).toHaveTextContent("Upload Pipeline");
  })
  ...
  it('should enable next button on upload model option selection and navigate to new model upload page on click', () => {
    ...
    const router = useRouter();
    render(<MockProviders><NewModelModule/></MockProviders>);
    expect(screen.getByTestId("newmodel-next-button")).toHaveTextContent("Next >");
    expect(screen.getByTestId("newmodel-next-button")).toHaveAttribute('aria-disabled', 'true');
    const option = screen.getByTestId("new-model-option");
    act(() => {
      option.dispatchEvent(new MouseEvent('click', {bubbles: true}));
    });
    expect(screen.getByTestId("newmodel-next-button")).not.toHaveAttribute('aria-disabled', 'true');
